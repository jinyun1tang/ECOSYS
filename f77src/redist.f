...
C
     ...
     1,THGFLG(JZ,JY,JX),THGFLS(JZ,JY,JX)
     2,OMCL(0:JZ,JY,JX),OMNL(0:JZ,JY,JX),EFIRE(2,21:22)
     3,ONL(4,0:4),OPL(4,0:4),DDLYR(5),DDLYX(5),IFLGL(5)
     3,DVOLW(JZ,JY,JX),DVOLI(JZ,JY,JX),VHCPWZ(JZ,JY,JX)
...
C
      ...
      IF(((L.EQ.0.AND.BKDS(L+1,NY,NX).LT.ZERO
     2.AND.VOLW(L+1,NY,NX).GT.ZEROS(NY,NX))
     2.OR.(L.GT.0.AND.BKVL(L,NY,NX).LT.ZERO))
...
C
      IF(IERSNG.NE.0.AND.N.NE.3)THEN
...
C
     ...
     2+1.9274*VOLI(0,NY,NX)
      IF(VHCP(0,NY,NX).GT.ZEROS(NY,NX))THEN
...
C    2,OSGX,ORGC(0,NY,NX),FLWR(NY,NX),FLSWR(NY,NX),TQR(NY,NX) 
...
C     WRITE(*,5456)'TLNH40',I,J,NX,NY,TLNH4
...
C
     ...
     2+1.9274*(VOLI(L,NY,NX)+VOLIH(L,NY,NX))
      VOLW1=VOLW(L,NY,NX)+VOLWH(L,NY,NX)
      VOLI1=VOLI(L,NY,NX)+VOLIH(L,NY,NX)
     ...
     2+TTHAWH(L,NY,NX)
      ...
      DVOLW(L,NY,NX)=VOLW1-VOLW(L,NY,NX)-VOLWH(L,NY,NX)
      DVOLI(L,NY,NX)=VOLI1-VOLI(L,NY,NX)-VOLIH(L,NY,NX)
...
C    5,TFLW(L,NY,NX),FLW(3,L,NY,NX),FLW(3,L+1,NY,NX),FLSW(NY,NX)
C    2,FINH(L,NY,NX),TTHAW(L,NY,NX),TUPWTR(L,NY,NX)
C    3,FLU(L,NY,NX),TQR(NY,NX) 
C    4,PSISM(L,NY,NX),VOLP(L,NY,NX)
C    5,VOLA(L,NY,NX),VOLX(L,NY,NX),VOLT(L,NY,NX)
...
C    7,(HFLW(3,LL,NY,NX),LL=1,10)
6633  FORMAT(A8,5I4,40F18.6)
C     ENDIF
C
...
C     WRITE(*,5432)'CH4SL',I,J,NX,NY,L,CH4S(L,NY,NX),TCHFLS(L,NY,NX)
...
C     WRITE(*,5432)'OXYSL',I,J,NX,NY,L,OXYS(L,NY,NX) 
...
C
     ...
     2-XN3BBL(L,NY,NX)-XNBBBL(L,NY,NX)
      ...
      TNBP(NY,NX)=TNBP(NY,NX)+CH+12.0*TBCO2(L,NY,NX)
      OXYGIN=OXYGIN+OIB
...
C    3,ZNH3G(L,NY,NX)
C     WRITE(*,5455)'TLNH4L',I,J,NX,NY,L,TLNH4
...
C     IF(I.EQ.187)THEN
C     DO 4344 K=0,4
C     WRITE(*,4343)'ORGC',I,J,NX,NY,L,K,ORGC(L,NY,NX),DC,OC
...
C     WRITE(*,4343)'ORGN',I,J,NX,NY,L,K,ORGN(L,NY,NX),RN,ON
...
C     SOIL SUBSIDENCE
C
      DO 225 L=NU(NY,NX),NL(NY,NX)-1
C
C     POND
C
      IF(BKDS(L,NY,NX).LE.ZERO.AND.BKDS(L+1,NY,NX).LE.ZERO)THEN
      DDLYX1=(VOLAI(L,NY,NX)-VOLW(L,NY,NX)-VOLI(L,NY,NX))
     2/AREA(3,L,NY,NX)
      DPTWI=(VOLW(L+1,NY,NX)+VOLI(L+1,NY,NX))/AREA(3,L,NY,NX)
      IF(DDLYX1.LT.-ZERO.OR.DPTWI.GT.ZEROS(NY,NX))THEN
      DDLYX(1)=DDLYX1
      DDLYR(1)=AMIN1(DDLYX(1),DPTWI)
      IFLGL(1)=1
      ELSE
      DDLYX(1)=(VOLA(L,NY,NX)-VOLW(L,NY,NX)-VOLI(L,NY,NX))
     2/AREA(3,L,NY,NX)
      DDLYR(1)=DDLYX(1)
      IFLGL(1)=2
      ENDIF
...
C     SOIL
C
      ELSEIF(BKDS(L,NY,NX).GT.ZERO)THEN
C
C     FREEZE-THAW 
C
      DDLYX1=(VOLA(L,NY,NX)-VOLW(L,NY,NX)-VOLI(L,NY,NX)
     2+VOLAH(L,NY,NX)-VOLWH(L,NY,NX)-VOLIH(L,NY,NX))
     2/AREA(3,L,NY,NX)
      IF(DDLYX1.LT.-ZERO.AND.DVOLI(L,NY,NX).LT.-ZEROS(NY,NX))THEN
C     DDLYX(4)=AMAX1(DDLYX1,DVOLI(L,NY,NX)*DENSJ)/AREA(3,L,NY,NX)
      DDLYX(4)=DVOLI(L,NY,NX)*DENSJ/AREA(3,L,NY,NX)
C     DDLYX(4)=DDLYX1/AREA(3,L,NY,NX)
      DDLYR(4)=DDLYX(4)
      IFLGL(4)=0
      ELSEIF(DDLYX1.GT.ZERO.AND.DVOLI(L,NY,NX).GT.ZEROS(NY,NX)
     2.AND.BKDS(L,NY,NX).LT.BKDSM(L,NY,NX))THEN
      DDLYX(4)=AMIN1(DVOLI(L,NY,NX)*DENSJ/AREA(3,L,NY,NX)
     2,(BKDSM(L,NY,NX)-BKDS(L,NY,NX))/BKDSM(L,NY,NX)*DLYR(3,L,NY,NX))
C     DDLYX(4)=AMIN1(DDLYX1/AREA(3,L,NY,NX)
C    2,(BKDSM(L,NY,NX)-BKDS(L,NY,NX))/BKDSM(L,NY,NX)*DLYR(3,L,NY,NX))
      DDLYR(4)=DDLYX(4)
      IFLGL(4)=0
      ELSE
      DDLYX(4)=0.0
      ...
      IFLGL(4)=0
      ENDIF
C     IF(DDLYR(4).NE.0.0)THEN
C     WRITE(*,1111)'FREEZ',I,J,NX,NY,L,DDLYR(4)
C    2,DDLYX1,DVOLI(L,NY,NX)*DENSJ,(BKDSM(L,NY,NX)
C    3-BKDS(L,NY,NX))/BKDSM(L,NY,NX)*DLYR(3,L,NY,NX)
C    4,DVOLI(L,NY,NX),DVOLW(L,NY,NX),TFLW(L,NY,NX),FINH(L,NY,NX)
C    2,TTHAW(L,NY,NX),TUPWTR(L,NY,NX),FLU(L,NY,NX)
C    2,VOLI(L,NY,NX),VOLP(L,NY,NX) 
C    4,VOLXI(L,NY,NX),VOLX(L,NY,NX),VOLA(L,NY,NX),VOLW(L,NY,NX)
C    3,VOLAH(L,NY,NX),VOLWH(L,NY,NX),VOLIH(L,NY,NX)
C    3,BKDSI(L,NY,NX),BKDS(L,NY,NX),BKDSM(L,NY,NX)
C    4,DLYR(3,L,NY,NX),CDPTH(0,NY,NX),DTBLX(NY,NX)
C     ENDIF 
C
C     EROSION
C
      IF(IERSNG.NE.0.AND.ABS(TSEDER(NY,NX)).GT.ZERO)THEN
      DDLYX1=-TSEDER(NY,NX)/(AREA(3,L,NY,NX)*BKDS(NU(NY,NX),NY,NX))
      IF(DDLYX1.LT.-ZERO.OR.VOLX(L+1,NY,NX).GT.ZEROS(NY,NX))THEN
      DDLYX(5)=DDLYX1
      DDLYR(5)=AMIN1(DDLYX1,VOLX(L+1,NY,NX))
      IFLGL(5)=1
      ELSE
      DDLYX(5)=DDLYX1
      DDLYR(5)=DDLYX(5)
      ...
      IFLGL(5)=0
      ENDIF
C     IF(DDLYR(5).NE.0.0)THEN
C     WRITE(*,1111)'SED',I,J,NX,NY,L,DDLYX(5),DDLYR(5)
C    2,DDLYX1,TSEDER(NY,NX),BKDS(L,NY,NX),VOLXI(L,NY,NX) 
C    2,VOLX(L,NY,NX) 
1111  FORMAT(A8,5I6,40E14.6)
C     ENDIF
C
C     DEFAULT NO LAYER CHANGE
C
      ELSE
      DDLYX(1)=0.0
      DDLYR(1)=0.0
...
C
      ...
      IF(IFLGL(NN).EQ.1)THEN
      CDPTH(L,NY,NX)=CDPTH(L,NY,NX)+DDLYR(NN)
C     WRITE(*,1117)'CDPTH1',I,J,NX,NY,L,NN,DDLYR(NN)
C    2,CDPTH(L,NY,NX)
1117  FORMAT(A8,6I6,12E14.6)
      ELSEIF(IFLGL(NN).EQ.2)THEN 
      DO 210 LL=L,0,-1
      CDPTH(LL,NY,NX)=CDPTH(LL,NY,NX)+DLYR(3,L+1,NY,NX)
C     WRITE(*,1116)'CDPTH2',I,J,NX,NY,L,LL,NN,DDLYR(NN)
C    2,CDPTH(LL,NY,NX)
210   CONTINUE
      ELSEIF(IFLGL(NN).EQ.0.AND.DDLYR(NN).NE.0.0)THEN 
      DO 220 LL=L-1,0,-1
      CDPTH(LL,NY,NX)=CDPTH(LL,NY,NX)+DDLYR(NN)
      IF(LL.GT.0)THEN
      DLYRX=CDPTH(LL,NY,NX)-CDPTH(LL-1,NY,NX)
      VOLTX=AREA(3,LL,NY,NX)*DLYRX
      VOLXX=VOLTX*FMPR(LL,NY,NX)
      VOLAX=POROS(LL,NY,NX)*VOLXX
C     WRITE(*,1116)'CDPTH0',I,J,NX,NY,L,LL,NN,DDLYR(NN)
C    2,CDPTH(LL,NY,NX),TSEDER(NY,NX),VOLXI(L,NY,NX)
C    2,VOLX(L,NY,NX),VOLA(L,NY,NX),VOLW(L,NY,NX)
C    2,VOLI(L,NY,NX),BKDS(L,NY,NX),BKDSI(L,NY,NX)
C    3,DLYRX,VOLAX,VOLW(LL,NY,NX)+VOLI(LL,NY,NX)
C    4,VOLAX-VOLW(LL,NY,NX)-VOLI(LL,NY,NX)
      ENDIF
220   CONTINUE
      ENDIF
      IF(IFLGL(NN).NE.0)THEN
      IF(BKDS(L,NY,NX).LT.ZERO)THEN
      IF(VOLW(L,NY,NX)+VOLI(L,NY,NX).GT.0.0)THEN
      DDLYZ=(DVOLW(L,NY,NX)+DVOLI(L,NY,NX))/AREA(3,L,NY,NX)
      ELSE
      DDLYZ=0.0
      ENDIF
      ...
      DDLYZ=0.0
      ENDIF 
      IF(DDLYZ.NE.0.0)THEN
      DO 215 LL=L-1,0,-1
      CDPTH(LL,NY,NX)=CDPTH(LL,NY,NX)+DDLYZ 
C     WRITE(*,1116)'CDPTHL',I,J,NX,NY,L,LL,NN,DDLYR(NN),DDLYZ
C    2,CDPTH(LL,NY,NX)
1116  FORMAT(A8,7I6,30E14.6)
215   CONTINUE
      ENDIF
      ENDIF
      ENDIF
200   CONTINUE
      ...
      ENDIF
C     IF(TSEDER(NY,NX).NE.0.0)THEN
C     WRITE(*,1114)'DDLYR',I,J,NX,NY,L,IFLGL(1),IFLGL(4),IFLGL(5)
C    2,DDLYX(1),DDLYX(4),DDLYX(5),TSEDER(NY,NX),TSED(NY,NX)
C    4,VOLXI(L,NY,NX),VOLX(L,NY,NX),VOLA(L,NY,NX),VOLW(L,NY,NX)
C    4,VOLI(L,NY,NX),VOLP(L,NY,NX),DVOLW(L,NY,NX),DVOLI(L,NY,NX)
C    5,DLYR(3,L,NY,NX),CDPTH(L-1,NY,NX),CDPTH(L,NY,NX) 
C    5,BKDS(L,NY,NX),BKDSI(L,NY,NX),POROS(L,NY,NX) 
C    6,VOLAH(L,NY,NX),VOLWH(L,NY,NX),VOLIH(L,NY,NX) 
C    7,DTBLZ(NY,NX),DTBLX(NY,NX) 
1114  FORMAT(A8,8I4,40E14.6)
C     ENDIF
C
C     RESET SURFACE LAYER NUMBER IF LOST TO EVAPORATION, EROSION
C
     ...
     3.OR.NUM(NY,NX).GT.NU(NY,NX)))THEN
      ...
      ENDIF
C     WRITE(*,5598)'SURFX',I,J,L,LL,NX,NY,NUX,NU(NY,NX),NUM(NY,NX) 
C    2,DDLYR(2),VOLX(NUX,NY,NX),VOLW(NUX,NY,NX),VOLI(NUX,NY,NX)
C    2,VHCP(NUX,NY,NX),VHCPNX(NY,NX) 
5598  FORMAT(A8,9I4,12E14.6)
      GO TO 9971
      ENDIF
9970  CONTINUE
      ELSE
...
9971  CONTINUE
C
C     RESET SURFACE LAYER NUMBER IF GAIN FROM PRECIPITATION
C
     ...
     3.OR.(BKDS(L,NY,NX).LE.ZERO))THEN
      NU(NY,NX)=NUI(NY,NX)
      NUM(NY,NX)=NUI(NY,NX)
      DDLYR(3)=(VOLWD(NY,NX)-TVOLWX)/AREA(3,0,NY,NX)
      DDLYX(3)=DDLYR(3)
      IFLGL(3)=1
      DLYRX=VOLX(0,NY,NX)/AREA(3,0,NY,NX)
      DLYR(3,0,NY,NX)=DLYRX+DDLYR(3) 
      DLYR(3,NU(NY,NX),NY,NX)=DLYR(3,NU(NY,NX),NY,NX)-DDLYR(3) 
...
C     IF(BKDS(NU(NY,NX),NY,NX).LE.ZERO)THEN
C     VOLT(NU(NY,NX),NY,NX)=DLYR(3,NU(NY,NX),NY,NX)
C    2*AREA(3,NU(NY,NX),NY,NX)
C     VOLX(NU(NY,NX),NY,NX)=VOLT(NU(NY,NX),NY,NX)
C    2*FMPR(NU(NY,NX),NY,NX)
C     ENDIF
C     WRITE(*,5597)'SURFY',I,J,L,NX,NY,NUI(NY,NX),NU(NY,NX)
C    2,NUM(NY,NX),VOLWD(NY,NX),TVOLWX,DDLYR(3),DLYRX 
...
C
      DO 230 NN=1,5
      IF(ABS(DDLYR(NN)).GT.ZERO)IFLGS(NY,NX)=1
      IF(IFLGL(NN).EQ.1)THEN
      IF(ABS(DDLYR(NN)).GT.ZERO)THEN
      IF(DDLYR(NN).GT.0.0)THEN
      IF(NN.NE.2)THEN
      L1=L
      L0=L+1
      ELSE
      ...
      L0=NUX
      ENDIF
      IF(DDLYR(NN).LT.DDLYX(NN))THEN
      FX=1.0
      ELSE
      IF(BKDS(L0,NY,NX).LT.ZERO)THEN
      DPTWI=(VOLW(L0,NY,NX)+VOLI(L0,NY,NX))/AREA(3,L0,NY,NX)
      FX=AMIN1(1.0,DDLYR(NN)/DPTWI)
      ELSE
      FX=AMIN1(1.0,DDLYR(NN)/DLYR(3,L0,NY,NX))
      ENDIF
      ENDIF
      ELSE
      IF(NN.NE.3)THEN
...
C
     ...
     2-CDPTH(L,NY,NX).LT.ZERO)THEN
      DLYR(3,NL(NY,NX),NY,NX)=0.0
      NL(NY,NX)=L
C     WRITE(*,5595)'ERSNX',I,J,NX,NY,L,NLI(NY,NX),NL(NY,NX)
...
5595  FORMAT(A8,7I4,12E14.6)
      ...
      ENDIF
      ENDIF
      ELSEIF(NN.EQ.1.AND.IFLGL(NN).EQ.2)THEN
...
230   CONTINUE
225   CONTINUE 
      DLYR(3,NL(NY,NX),NY,NX)=CDPTH(NL(NY,NX),NY,NX)
     2-CDPTH(NL(NY,NX)-1,NY,NX)
...
C    2,TLPO4,TPIN,TPOU,TORGP,XPSN,Z1PW(NY,NX),ZHPW(NY,NX)
