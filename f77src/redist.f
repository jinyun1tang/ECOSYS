...
C
     ...
     2,ONL(4,0:4),OPL(4,0:4),DDLYR(3),DDLYX(3),IFLGL(3)
     3,DVOLW(JZ,JY,JX),DVOLI(JZ,JY,JX)
      DATA SG/0.0/
...
C
      ...
      Z1PW(NY,NX)=Z1PW(NY,NX)+XH1PBS(NY,NX)
...
C     WRITE(*,444)'ZOHW',I,J,NX,NY,ZOHW(NY,NX)
C    2,XOHBLS(NY,NX),TQSOH(NY,NX)
444   FORMAT(A8,4I4,12E12.4)
...
C
C     IF(J.EQ.24)THEN
      WS=VOLSS(NY,NX)+VOLWS(NY,NX)+VOLIS(NY,NX)*DENSI
      VOLWSO=VOLWSO+WS
      ...
      TLH2G=TLH2G+H2GW(NY,NX)
      TLN2G=TLN2G+ZNGW(NY,NX)+ZN2W(NY,NX)
      TLNH4=TLNH4+ZN4W(NY,NX)+ZN3W(NY,NX)
      TLNO3=TLNO3+ZNOW(NY,NX)
      TLPO4=TLPO4+Z1PW(NY,NX)+ZHPW(NY,NX)
      IF(ISALT(NY,NX).NE.0)THEN
      SSW=ZALW(NY,NX)+ZFEW(NY,NX)+ZHYW(NY,NX)+ZCAW(NY,NX)
     ...
     4+ZCA2PW(NY,NX))
     5+5.0*(ZALH4W(NY,NX)+ZFEH4W(NY,NX)) 
      TION=TION+SSW
      ENDIF
C     ENDIF
C
...
C
      ...
      DC=DC+OQC(K,0,NY,NX)+OQA(K,0,NY,NX)
      DN=DN+OQN(K,0,NY,NX)
      DP=DP+OQP(K,0,NY,NX)
      OQC(K,0,NY,NX)=0.0
      OQN(K,0,NY,NX)=0.0
      ...
      DC=DC+OQCH(K,0,NY,NX)+OQAH(K,0,NY,NX)
      DN=DN+OQNH(K,0,NY,NX)
      DP=DP+OQPH(K,0,NY,NX)
      OQCH(K,0,NY,NX)=0.0
      OQNH(K,0,NY,NX)=0.0
      OQPH(K,0,NY,NX)=0.0
      OQAH(K,0,NY,NX)=0.0
...
C
      ...
      DC=DC+OHC(K,0,NY,NX)+OHA(K,0,NY,NX)
      DN=DN+OHN(K,0,NY,NX)
      DP=DP+OHP(K,0,NY,NX)
      OHC(K,0,NY,NX)=0.0
      OHN(K,0,NY,NX)=0.0
...
C    2,TQR(N2,N1),QR(N,N2,N1),QR(N,N5,N4)
...
C
     ...
     2+TQR(NY,NX) 
      ...
      ENGYR=VHCP(0,NY,NX)*TKS(0,NY,NX)-HFLXD
     ...
     2+THQR(NY,NX))/VHCP(0,NY,NX)
      ...
      ENDIF
      IF(VHCP(0,NY,NX).LT.VHCPRX(NY,NX))THEN
      HFLXR=VHCP(0,NY,NX)*(TKS(0,NY,NX)-TKS(NUM(NY,NX),NY,NX))
      HEATOU=HEATOU+HFLXR
      TKS(0,NY,NX)=TKS(NUM(NY,NX),NY,NX)
      ENDIF
...
C     IF(NX.EQ.1)THEN
C     WRITE(*,5151)'VOLW0',I,J,NX,NY,VOLW(0,NY,NX) 
C    2,VOLI(0,NY,NX),FLWR(NY,NX),THAWR(NY,NX),TQR(NY,NX)
C     WRITE(*,5151)'TK0',I,J,NX,NY,TKS(0,NY,NX),ENGYR 
C    2,HFLWR(NY,NX),HFLXD,HTHAWR(NY,NX),VHCP(0,NY,NX) 
C    3,ORGC(0,NY,NX),VHCP(0,NY,NX)*TKS(0,NY,NX)
...
C     WRITE(20,3338)'SIR',I,J,SIR,PRECQ(NY,NX)
...
C
      ...
      Z1PW(NY,NX)=Z1PW(NY,NX)+TP1QSS(NY,NX)
...
C
     ...
     2/(AREA(3,NU(NY,NX),NY,NX)*BKDS(NU(NY,NX),NY,NX))
      VOLX(NU(NY,NX),NY,NX)=AREA(3,NU(NY,NX),NY,NX)
     2*DLYR(3,NU(NY,NX),NY,NX)*FMPR(NU(NY,NX),NY,NX)
      ENDIF
C
...
C
     ...
     2+1.9274*(VOLI(NUM(NY,NX),NY,NX)+VOLIH(NUM(NY,NX),NY,NX))
      VOLI(NUM(NY,NX),NY,NX)=VOLI(NUM(NY,NX),NY,NX)+FLWSI(NY,NX)
      ENGY=VHCP(NUM(NY,NX),NY,NX)*TKS(NUM(NY,NX),NY,NX)
      VHCP(NUM(NY,NX),NY,NX)=VHCM(NUM(NY,NX),NY,NX)
     2+4.19*(VOLW(NUM(NY,NX),NY,NX)+VOLWH(NUM(NY,NX),NY,NX))
     2+1.9274*(VOLI(NUM(NY,NX),NY,NX)+VOLIH(NUM(NY,NX),NY,NX))
      TKS(NUM(NY,NX),NY,NX)=(ENGY+HFLWSI(NY,NX))/VHCP(NUM(NY,NX),NY,NX)
      ENDIF
...
C
     ...
     2+TTHAW(L,NY,NX)+TUPWTR(L,NY,NX)+FLU(L,NY,NX) 
      DVOLW(L,NY,NX)=VOLW1-VOLW(L,NY,NX)
     ...
     2,AMIN1(VOLW(L,NY,NX),VOLWX(L,NY,NX)))
      ...
      DVOLI(L,NY,NX)=VOLI1-VOLI(L,NY,NX)
...
C     IF(L.EQ.1)THEN
C     WRITE(*,6547)'VOLW',I,J,NX,NY,L,VOLW(L,NY,NX),DVOLW(L,NY,NX)
C    2,TFLW(L,NY,NX),FINH(L,NY,NX),TTHAW(L,NY,NX),TUPWTR(L,NY,NX)
C    3,FLU(L,NY,NX),TQR(NY,NX),VOLI1,DENSI 
C    4,PSISM(L,NY,NX),VOLI(L,NY,NX),DVOLI(L,NY,NX),VOLP(L,NY,NX)
C    5,VOLA(L,NY,NX),VOLX(L,NY,NX)
C    5,FLW(3,L,NY,NX),FLW(3,L+1,NY,NX),FLW(1,L,NY,NX+1)
...
C    6,FLWSI(NY,NX),HFLWSI(NY,NX),HFLW(3,L,NY,NX),HFLW(3,L+1,NY,NX)
6633  FORMAT(A8,5I4,30E12.4)
C     ENDIF
C
...
C     WRITE(*,5432)'OXYSL',I,J,NX,NY,L,OXYS(L,NY,NX),TOXFLS(L,NY,NX)
C    2,XOXDFG(L,NY,NX),RUPOXO(L,NY,NX),TUPOXS(L,NY,NX)
C    3,ROXFLU(L,NY,NX),XOXFXS(L,NY,NX),XOXBBL(L,NY,NX),COXYS(L,NY,NX)
C    4,XOXFLS(3,L,NY,NX),XOXFLS(3,L+1,NY,NX),XOXFLS(1,L,NY,NX+1)
C    5,XOXDFS(NY,NX)
...
C     ENDIF
     ...
     4+ZFE2PH(L,NY,NX)+ZCA2PH(L,NY,NX)+H3POBH(L,NY,NX)+ZFE2BH(L,NY,NX)
     5+ZCA2BH(L,NY,NX))
...
C
      DO 225 L=NU(NY,NX),NL(NY,NX)
      IF(IERSN(NY,NX).NE.0.AND.L.EQ.NU(NY,NX)
     2.AND.BKDS(NU(NY,NX),NY,NX).GT.ZERO)THEN
      DDLYX(1)=(VOLXI(L,NY,NX)-VOLX(L,NY,NX))/AREA(3,L,NY,NX)
      DDLYR(1)=DDLYX(1)
      IFLGL(1)=1
      ELSEIF(BKDS(L,NY,NX).LE.ZERO.AND.BKDS(L+1,NY,NX).LE.ZERO)THEN
      DDLYX1=(VOLAI(L,NY,NX)-VOLW(L,NY,NX)-VOLI(L,NY,NX))
     2/AREA(3,L,NY,NX)
      VOLWI=VOLW(L+1,NY,NX)+VOLI(L+1,NY,NX)
      IF(DDLYX1.LT.-ZERO.OR.VOLWI.GT.0.0)THEN
      DDLYX(1)=DDLYX1
      DDLYR(1)=AMIN1(DDLYX(1),(VOLWI/AREA(3,L,NY,NX)))
      IFLGL(1)=1
      ELSE
     ...
     2/AREA(3,L,NY,NX)
      ...
      IFLGL(1)=2
      ENDIF
      ELSEIF(BKDS(L,NY,NX).GT.ZERO.AND.CORGC(L,NY,NX).GT.FORGC)THEN
      DDLYX(1)=(VOLA(L,NY,NX)-VOLW(L,NY,NX)-VOLI(L,NY,NX))
     2/AREA(3,L,NY,NX)
      IF(DDLYX(1).LT.-ZERO.OR.BKDS(L,NY,NX).LT.BKDX(L,NY,NX))THEN
      DDLYR(1)=DDLYX(1)
      ELSE
      DDLYX(1)=0.0
      DDLYR(1)=0.0
...
C
      IF(IFLGL(1).EQ.1)THEN
      IF(L.EQ.NU(NY,NX).AND.BKDS(L,NY,NX).LE.ZERO)THEN
      CDPTH(L-1,NY,NX)=CDPTH(L,NY,NX)
     2-DLYRI(3,L,NY,NX)+DDLYR(1)
      ENDIF
      CDPTH(L,NY,NX)=CDPTH(L,NY,NX)+DDLYR(1)
      ELSEIF(IFLGL(1).EQ.0)THEN 
      DO 220 LL=L-1,0,-1
      CDPTH(LL,NY,NX)=CDPTH(LL,NY,NX)+DDLYR(1)
220   CONTINUE
      ENDIF
      IF(IFLGL(1).NE.0.AND.VOLW(L,NY,NX)+VOLI(L,NY,NX).GT.0.0)THEN
      DDLYZ=AMIN1((DVOLW(L,NY,NX)+DVOLI(L,NY,NX))/AREA(3,L,NY,NX)
     2,CDPTH(L,NY,NX)-CDPTH(L-1,NY,NX))
      IF(DDLYR(1).LT.DDLYX(1))THEN
      DDLYZ=DDLYZ-VOLW(L+1,NY,NX)-VOLI(L+1,NY,NX)
      ENDIF
      DO 215 LL=L-1,0,-1
...
215   CONTINUE
      ...
      ENDIF
C     IF(IYRC.EQ.2006.AND.I.EQ.361.AND.NX.EQ.1)THEN
C     WRITE(*,1114)'DDLYR',I,J,NX,NY,L,IFLGL(1),DDLYR(1),DDLYX(1)
C    4,VOLAI(L,NY,NX),VOLA(L,NY,NX),VOLW(L,NY,NX),VOLI(L,NY,NX)
C    4,VOLW(L,NY,NX)+VOLI(L,NY,NX),DLYR(3,L-1,NY,NX),DLYR(3,L,NY,NX) 
C    4,CDPTH(L-1,NY,NX),CDPTH(L,NY,NX) 
C    3,CDPTHZ(L-1,NY,NX),CDPTHZ(L,NY,NX) 
C    5,POROS(L-1,NY,NX),POROS(L,NY,NX)
C    5,VOLA(L-1,NY,NX),VOLA(L,NY,NX)
C    5,VOLX(L-1,NY,NX),VOLX(L,NY,NX)
C    5,VOLW(L-1,NY,NX),VOLW(L,NY,NX) 
C    5,BKDX(L,NY,NX),BKDS(L,NY,NX),CORGC(L,NY,NX)
C    6,VOLAH(L,NY,NX),VOLWH(L,NY,NX) 
1114  FORMAT(A8,6I4,30E14.6)
C     ENDIF
C
C     RESET SURFACE LAYER NUMBER IF LOST TO EVAPORATION, EROSION
C
      IF(VOLX(NU(NY,NX),NY,NX).LE.ZEROS(NY,NX)
     2.OR.NUM(NY,NX).GT.NU(NY,NX))THEN
      NUX=NU(NY,NX)
      ...
      ENDIF
      WRITE(*,5598)'SURFX',I,J,L,NX,NY,NUX,NU(NY,NX),NUM(NY,NX) 
     2,DDLYR(2),VOLX(NUX,NY,NX) 
5598  FORMAT(A8,8I4,12E14.6)
      GO TO 9971
      ENDIF
9970  CONTINUE
      ELSE
...
C
     ...
     2.AND.((BKDS(L,NY,NX).GT.ZERO.AND.NU(NY,NX).GT.NUI(NY,NX)
     2.AND.TVOLG(NY,NX).GT.VOLWD(NY,NX))
     3.OR.BKDS(L,NY,NX).LE.ZERO
     4.AND.TVOLG(NY,NX).GT.ZEROS(NY,NX)))THEN 
      NU(NY,NX)=NUI(NY,NX)
      NUM(NY,NX)=NUI(NY,NX)
      DDLYR(3)=-1.0
      DDLYX(3)=-1.0
      IFLGL(3)=1
      DDLYX1=-(VOLW(0,NY,NX)+VOLI(0,NY,NX)+VOLW(NU(NY,NX),NY,NX)
     2+VOLI(NU(NY,NX),NY,NX))/AREA(3,NU(NY,NX),NY,NX)
      CDPTH(0,NY,NX)=CDPTH(NU(NY,NX),NY,NX)+DDLYX1-DDLYR(1) 
      DLYR(3,NU(NY,NX),NY,NX)=CDPTH(NU(NY,NX),NY,NX)-CDPTH(0,NY,NX) 
     ...
     2+CDPTH(0,NY,NX))
      ...
      VOLT(NU(NY,NX),NY,NX)=AREA(3,NU(NY,NX),NY,NX)
     2*DLYR(3,NU(NY,NX),NY,NX)
      VOLX(NU(NY,NX),NY,NX)=VOLT(NU(NY,NX),NY,NX)
     2*FMPR(NU(NY,NX),NY,NX)
      ENDIF
      DLYR(3,0,NY,NX)=0.0
      VOLT(0,NY,NX)=0.0
      VOLX(0,NY,NX)=VOLT(0,NY,NX) 
      VOLXI(0,NY,NX)=VOLX(0,NY,NX)
      WRITE(*,5598)'SURFY',I,J,L,NX,NY,NUI(NY,NX),NU(NY,NX),NUM(NY,NX)
     2,DDLYX1,DLYR(3,NU(NY,NX),NY,NX),ORGC(0,NY,NX),VOLW(0,NY,NX)
     2,VOLW(NU(NY,NX),NY,NX),DDLYR(3),TVOLG(NY,NX),VOLWD(NY,NX)
     3,CDPTH(0,NY,NX),CDPTH(NU(NY,NX),NY,NX),CDPTHI(NY,NX)
      ELSE
      DDLYR(3)=0.0
      DDLYX(3)=0.0
      IFLGL(3)=0
...
C
      DO 230 NN=1,3
      IF(DDLYR(NN).NE.0.0)IFLGS(NY,NX)=1
      IF(IFLGL(NN).EQ.1)THEN
      IF(DDLYR(NN).NE.0.0)THEN
      IF(DDLYR(NN).GT.0.0)THEN
      IF(DDLYR(NN).LT.1.0)THEN
      L1=L
      L0=L+1
      IF(DDLYR(NN).LT.DDLYX(NN))THEN
      FX=1.0
      ELSE
      VOLWI=VOLW(L0,NY,NX)+VOLI(L0,NY,NX)
      FX=AMIN1(1.0,DDLYR(NN)/(VOLWI/AREA(3,L0,NY,NX)))
      ENDIF
      ELSE
      L1=NU(NY,NX)
      L0=NUX
...
C    5,VLNH4(L0,NY,NX),VLNH4(L1,NY,NX)
...
C
     ...
     2.OR.FX*DLYR(3,L0,NY,NX).GT.ZERO)THEN
      BKDX(L1,NY,NX)=(BKDX(L1,NY,NX)
     2*DLYR(3,L1,NY,NX)+BKDX(L0,NY,NX)
     ...
     2+FX*VOLIH(L0,NY,NX)
C     VOLAH(L1,NY,NX)=VOLAH(L1,NY,NX)
C    2+FX*VOLAH(L0,NY,NX)
      ENDIF
     ...
     2+FX*VOLP(L0,NY,NX)
C     VOLA(L1,NY,NX)=VOLA(L1,NY,NX)
C    2+FX*VOLA(L0,NY,NX)
...
3940  CONTINUE
      ...
      H1PO4(0,NY,NX)=H1PO4(0,NY,NX)*XCORP(NY,NX)
      ...
      VHCP(0,NY,NX)=VHCP(0,NY,NX)*XCORP(NY,NX)
...
C
      HEATSR=VHCPW(NY,NX)*TKW(NY,NX)+VHCP(0,NY,NX)*TKS(0,NY,NX)
     ...
     2-DLYR(3,L,NY,NX)))
      ...
      TI=TL/DLYR(3,L,NY,NX)
      TX=1.0-TI
      BKDX(L,NY,NX)=TI*(BKDX(L,NY,NX)+CORP*(TBKDX-BKDX(L,NY,NX)))
     2+TX*BKDX(L,NY,NX)
...
2000  CONTINUE
      ...
      ZNFNI(0,NY,NX)=ZNFNI(0,NY,NX)*XCORP(NY,NX)
...
C     WRITE(*,2222)'TLC',I,J,IYRC,NX,NY,TLRSDC+TLORGC+TLCO2G-CO2GIN
...
C    5,VOLSS(NY,NX),VOLWS(NY,NX),VOLIS(NY,NX)*DENSI,TQS(NY,NX)
...
C    2,HEATSO,HEATIN,HEATOU,HTHAWR(NY,NX),HFLXD,4.19*TKA(NY,NX)
...
C    6,VHCPW(NY,NX)*TKW(NY,NX),VHCP(0,NY,NX)*TKS(0,NY,NX)
