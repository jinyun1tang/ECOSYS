...
9984  CONTINUE
      ...
      VOLWU(L)=VOLWM(NPH,L,NY,NX)-THETY(L,NY,NX)*VOLX(L,NY,NX)
...
C
      ...
      IF(ILYR(N,L).EQ.1)THEN
      UPWTR(N,L,NZ,NY,NX)=AMAX1(AMIN1(0.0,-VOLWU(L)*FPQ(L,NZ))
     2,AMIN1((PSILC-PSIST1(L))/RSRS(N,L),VOLPU(L)*FPQ(L,NZ)))
...
C    4,VOLWU(L),THETY(L,NY,NX),VOLX(L,NY,NX),FPQ(L,NZ)
...
C
      ...
      FP14X=AMAX1(FPP(L,NZ),RUPP1P(N,L,NZ,NY,NX)/RP14Y(L,NY,NX))
      ELSE
      FP14X=FPQ(L,NZ)
      ENDIF
      IF(RP1BY(L,NY,NX).GT.ZEROS(NY,NX))THEN
      FP1BX=AMAX1(FPP(L,NZ),RUPP1B(N,L,NZ,NY,NX)/RP1BY(L,NY,NX))
      ELSE
      FP1BX=FPQ(L,NZ)
      ENDIF
      TFOXYX=TFOXYX+FOXYX
...
C    2,UPMXP,DIFOX,THETW1,THETM,RRADS,FPQ(L,NZ) 
...
C     WRITE(*,5547)'RUPNBX',I,J,NX,NY,NZ,L,N,RUPNBX,RDXNHB,RDFNHB
...
C     IF(I.EQ.105)THEN
C     WRITE(*,5566)'CO2P1',I,J,NX,NY,NZ,L,N,M,MX,RCO2SX
C    2,RCO2S(N,L,NZ,NY,NX),RDFCOS,RDXCOS,RMFCOS,DIFCL
C    3,CCO2S1,CCO2P1,RTVLW(N,L,NZ,NY,NX),CO2S1,VOLWMM
C    4,CO2P1,VOLWSP,PP(NZ,NY,NX),FPQ(L,NZ),RCODF1,RCO2PX
C    5,CO2PX,RTVLP(N,L,NZ,NY,NX),DFGP,VOLWCA,CO2A1
C    6,XNPD,PORT(N,NZ,NY,NX),TFND(L,NY,NX) 
