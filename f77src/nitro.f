...
C
     ...
     3,OQKAM=1.2E+01,CCKM=0.15,CCK4=1.2E-03,ZHKM=1.4,ZHKI=7.0E+03
...
C
     ...
     2,EOMN=75.0,GO2X=37.5,GH4X=66.5,GCHX=4.50,GO2A=GO2X-GCHX,GC4X=3.00
...
C    3,VOLW(L,NY,NX),VOLWH(L,NY,NX),VOLX(L,NY,NX),VOLT(L,NY,NX)
...
C    4,WFNG,TKS(L,NY,NX),PSISM(L,NY,NX),THETW(L,NY,NX) 
...
555   CONTINUE
     ...
     2+RIPO4(N,K)+RIPOB(N,K)+RIP14(N,K)+RIP1B(N,K) 
      ...
      ENDIF
C     IF(I.EQ.180.AND.J.EQ.24)THEN
C     WRITE(*,5556)'OMC3',IYRC,I,J,NX,NY,L,K,N,OMC(3,N,K,L,NY,NX)
C    2,OMN(3,N,K,L,NY,NX),OMP(3,N,K,L,NY,NX),OMC(1,N,K,L,NY,NX)
C    2,OMN(1,N,K,L,NY,NX),OMP(1,N,K,L,NY,NX),WFN(N,K),OXYI
C    2,COXYS(L,NY,NX),OXYI,WFN(N,K)
...
C    5,CGONS(M,N,K),R3OMN(M,N,K),R3MMN(M,N,K)
C    6,CGOMN(N,K),RINH4(N,K),RINB4(N,K),RINO3(N,K),RINB3(N,K)
C    7,RN2FX(N,K) 
5556  FORMAT(A8,8I4,20E12.4)
C     ENDIF
      ENDIF
545   CONTINUE
      ENDIF
