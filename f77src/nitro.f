...
C
     ...
     2,RINB4(7,0:5),RINB3(7,0:5),RIPOB(7,0:5),FOMK(7,0:5)
     ...
     7,DOSA(0:4),DOSX(0:4),DOSM(0:4),FORC(0:5),SPOMK(2),RMOMK(2) 
...
C
     ...
     4,OMGR=0.250,OQKI=1.2E+03,H2KI=1.0,OAKI=12.0,COMKI=1.0E-03
     5,COMKM=1.0E-04,CKC=1.0E-03,FOSCZ0=2.0E-02
     6,FOSCZL=5.0E-06,FMN=1.0E-03,DCKM0=5.0E+03,DCKML=1.0E+03)
C
C     SPECIFIC RESPIRATION RATES, M-M UPTAKE CONSTANTS,
C     STOICHIOMETRIC CONSTANTS FOR MICROBIAL REDOX REACTIONS
C
      PARAMETER (VMXO=0.125,VMXF=0.125,VMXM=0.125,VMXH=0.25,VMXN=0.25
     2,VMX4=0.25,VMXC=0.125,OQKM=1.2E+01,OQKA=1.2E+01,OQKAM=1.2E+01
     3,CCKM=0.15,CCK4=1.2E-04,ZHKM=1.4E-04,ZNKM=1.4,Z3KM=1.4
     4,Z2KM=1.4,Z1KM=0.014,Z4MX=5.0E-03,Z4KU=0.40,Z4MN=0.0125
     5,ZOMX=5.0E-03,ZOKU=0.35,ZOMN=0.03,HPMX=1.0E-03,HPKU=0.075 
     6,HPMN=0.002,ZFKM=0.14,H2KM=0.01,ECNH=0.30
     7,ECNO=0.10,ECN3=0.857,ECN2=0.857,ECN1=0.429,RNFNI=5.0E-04
     8,RNFNG=0.05,ECHO=0.75,VMKI=8.0,OXKA=0.32
...
C
      ...
      DATA SPOSC/7.5,7.5,1.25,0.25,7.5,7.5,1.25,0.25
     2,7.5,7.5,1.25,0.25,0.05,0.00,0.00,0.00
...
C
      ...
      ORGCX=AMIN1(1.0E+05*BKVL(L,NY,NX),ORGC(L,NY,NX))
C
C     NITRIFICATION INHIBITION
C
      IF(ZNFN0(L,NY,NX).GT.ZEROS(NY,NX))THEN
      TFNI=AMAX1(0.2,TFNX)
      ZNFNI(L,NY,NX)=AMAX1(0.0,ZNFNI(L,NY,NX)-RNFNI*TFNI)
      ZNFNG(L,NY,NX)=ZNFNG(L,NY,NX)+RNFNG*TFNI
     2*(1.0-ZNFNG(L,NY,NX)) 
...
C
      ...
      FNSHY=AMIN1(1.5,0.67+6.7*CNSHY)
      FPSHY=AMIN1(1.5,0.67+67.0*CPSHY)
      DO 760 K=0,5
      IF(L.NE.0.OR.(K.NE.3.AND.K.NE.4))THEN
      TCGOQC(K)=0.0
      TCGOAC(K)=0.0
...
C
      IF(ORGCX.GT.ZEROS(NY,NX))THEN
      DO 765 M=1,2
      COMC=OMC(M,N,K,L,NY,NX)/ORGCX
...
C     POTENTIAL RESPIRATION RATES 'RGO*P' WITH UNLIMITED SUBSTRATE 
C     USED FOR MICROBIAL COMPETITION FACTOR
C
...
C    5,FSBST,SPOMK(1),RMOMK(1),ROQCD(N,K)
...
C
      ...
      FCN3S=FNH4S*CNH3S(L,NY,NX)/(CNH3S(L,NY,NX)+ZHKM)
      FCN3B=FNHBS*CNH3B(L,NY,NX)/(CNH3B(L,NY,NX)+ZHKM)
      FSBST=FCN3S+FCN3B
...
C
      ...
      ROXYS(N,K,L,NY,NX)=ROXYP(N,K)
C     IF((I/1)*1.EQ.I.AND.J.EQ.14.AND.L.LE.5)THEN
...
C    8,TFNI,WFNG,ZNFNI(L,NY,NX),ZNFNG(L,NY,NX),ZNFNA 
C    9,SPOMK(1),RMOMK(1),BKVL(L,NY,NX) 
6666  FORMAT(A8,5I4,120E12.4)
C     ENDIF
C
C     NO2 OXIDIZERS
C
...
C    3,XCO2,FNO2,FNB2,VMXDS,VMXDB,TFNG(N,K),OMA(N,K),COMN,FCN2S,FCN2B
C    2,ROXYP(N,K),ROXYM(N,K),ROXYO(N,K),FOXYX,DFGS(NPH,L,NY,NX)
C    3,THETW(L,NY,NX),WFN(N,K),COXYS(L,NY,NX),COXYG(L,NY,NX)
C    4,ROXYD,ROXYD/VOLWZ,VMXDX,VMXDA,FNO2S,FNO2B
...
C
     ...
     2-OMN(3,N,K,L,NY,NX))/EN2F(N)
      RGN2F(N,K)=AMIN1(RGN2P,RGOMT,OMGR*OMC(3,N,K,L,NY,NX))
...
C
     ...
     2+OMP(3,N,K,L,NY,NX)/CPOMC(3,N,K)))
      ...
      ENDIF
C     IF((I/120)*120.EQ.I.AND.J.EQ.24.AND.L.LE.6)THEN
C     WRITE(*,5555)'RCCC',I,J,NX,NY,L,K,N,RCCC,CCC,C3C
C    2,OMC(3,N,K,L,NY,NX),OMN(3,N,K,L,NY,NX),OMP(3,N,K,L,NY,NX)
C    3,OMC(1,N,K,L,NY,NX),CXC
C     ENDIF
C
...
475   CONTINUE
     ...
     2,OSA(M,K,L,NY,NX)+DOSA(K)*(AMAX1(DOSM(K),AMIN1(DOSX(K) 
     3,TGROMC(K)/AREA(3,L,NY,NX))))*AREA(3,L,NY,NX) 
