...
C
     ...
     5,COMKM=1.0E-04,CKC=1.0E-03,FOSCZ0=2.0E-02,FOSCZL=5.0E-06
     6,FMN=1.0E-03,DCKM0=5.0E+03,DCKML=1.0E+03)
C
...
C
     ...
     3,OQKAM=1.2E+01,CCKM=0.15,CCK4=1.2E-04,ZHKM=1.4,ZHKI=7.0E+03
     ...
     8,ECNH=0.30,ECNO=0.10,ECN3=0.857,ECN2=0.857,ECN1=0.429
     9,RNFNI=2.0E-04,ECHO=0.75,VMKI=0.125,VHKI=15.0,OXKA=0.16
...
C     WRITE(*,8824)'THETZ',I,J,L,THETZ,THETR,VOLWZ,VOLWRX(NY,NX)
C    2,POROS(L,NY,NX),FCR(NY,NX),THETY(L,NY,NX)
C     ENDIF
      ELSE
      VOLWZ=0.0
      ENDIF
     ...
     2,THETW(L,NY,NX))-THETY(L,NY,NX)))
      VOLWZ=THETZ/(1.0+THETZ)*VOLX(L,NY,NX)
C     IF((I/30)*30.EQ.I.AND.J.EQ.24)THEN
C     WRITE(*,8824)'THETZ',I,J,L,THETZ,THETW(L,NY,NX),VOLWZ
C    2,VOLX(L,NY,NX),POROS(L,NY,NX),FC(L,NY,NX),THETY(L,NY,NX)
8824  FORMAT(A8,3I4,12E12.4)
C     ENDIF
      ENDIF
C
...
C    2,CNH4S(L,NY,NX),CNH4B(L,NY,NX),14.0*XN4(L,NY,NX),14.0*XNB(L,NY,NX)
...
C    4,ROXYD,VMXD4,VMXDXS,VMXDXB,VMXD4S,VMXD4B,FNO2S,FNO2B,ZNFN4S,ZNFN4B 
...
C     WRITE(*,7776)'RINH4',IYRC,I,J,NX,NY,L,K,N,RINH4(N,K),RINHP
...
C    4,ZNH4B(L,NY,NX),ZNH4T(L),OQN(K,L,NY,NX)
...
C     IF((I/30)*30.EQ.I.AND.J.EQ.24.AND.N.EQ.1.AND.M.EQ.1)THEN
...
C    2,(CGOQC(N,K),N=1,7),CSORP(K),OHC(K,L,NY,NX) 
...
C     WRITE(*,4441)'OMN2',I,J,NX,NY,L,K,N,M,OMN(M,N,K,L,NY,NX) 
...
C    5,OMN(3,N,K,L,NY,NX),CGONS(M,N,K),R3OMN(M,N,K),R3MMN(M,N,K)
...
C
C     IF(I.EQ.116)THEN
C     WRITE(*,336)'LAYER',I,J,L,(OQC(K,L,NY,NX),K=0,4)
...
C    2.AND.CDPTH(L,NY,NX).LE.CDPTH(NU(NY,NX)-1,NY,NX)+0.60)THEN
      ...
      IF(VOLX(LN,NY,NX).GT.ZEROS(NY,NX))THEN
      LL=LN
      GO TO 1101
      ENDIF
1100  CONTINUE
1101  CONTINUE
     ...
     2/(VOLT(L,NY,NX)+VOLT(LL,NY,NX))
      ...
      ENDIF
C     IF(I.GT.190.AND.NX.EQ.1)THEN
C     WRITE(*,1115)'MIX',I,J,NX,NY,L,LL,FOSCXS,FOSCZ0,FOSCZL,OSCXD,TOMA 
C    2,TFNX,ORGR(L,NY,NX),VOLT(LL,NY,NX),ORGR(LL,NY,NX),VOLT(L,NY,NX)
C    3,TKS(L,NY,NX),(OHC(K,L,NY,NX),K=0,4),(OHA(K,L,NY,NX),K=0,4)
