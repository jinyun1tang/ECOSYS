...
C
     ...
     7,ECNO=0.10,ECN3=0.857,ECN2=0.857,ECN1=0.429,RNFNI=0.00625
     8,RNFNG=0.0125,ECHO=0.75,VMKI=2.50,OXKA=0.32
...
C
     ...
     3,0.05,0.0167,0.00,0.00/
      ...
      DATA DCKM/0.5E+03,0.5E+03,0.5E+03,1.0E+03,1.0E+03/
...
C    2+AMAX1(0.0,H2POB(NU(NY,NX),NY,NX))
     ...
     2-THETY(L,NY,NX))*VOLWRX(NY,NX))
      ...
      ELSE
      KL=4
      VOLWZ=AMAX1(0.0,(AMIN1(AMAX1(0.10,FC(L,NY,NX)),THETW(L,NY,NX))
     2-THETY(L,NY,NX))*VOLX(L,NY,NX))
      ENDIF
C
...
C
      ...
      TKSM=AMAX1(258.15,TKS(L,NY,NX))+OFFSET(NY,NX)
      RTK=8.3143*TKSM
      STK=710.0*TKSM
...
C
      ...
      ZNFNI(L,NY,NX)=ZNFNI(L,NY,NX)-RNFNI
     2*ZNFNI(L,NY,NX)*AMAX1(1.0E-02
     3,1.0-SQRT(ZNFNI(L,NY,NX)/ZNFN0(L,NY,NX)))
      ZNFNG(L,NY,NX)=ZNFNG(L,NY,NX)+RNFNG
     2*(1.0-SQRT(ZNFNG(L,NY,NX))) 
...
C
      ...
      FNSHY=AMIN1(1.2,0.375+6.25*CNSHY)
      FPSHY=AMIN1(1.2,0.375+62.5*CPSHY)
      DO 760 K=0,5
      IF(L.NE.0.OR.(K.NE.3.AND.K.NE.4))THEN
      TCGOQC(K)=0.0
      TCGOAC(K)=0.0
...
C    3,VOLWM(NPH,L,NY,NX),FOSRH(K),DOMA,SPOMC2,FSBST(N,K),ROQCD(N,K)
...
C    6,FSBST(N,K),FOSRH(K),DOMA,SPOMC2,ROQCD(N,K)
...
C
      ...
      IF(VOLWZ.GT.ZEROS(NY,NX).AND.FOSRH(K).GT.ZERO)THEN
      VMXD3=VMXDX/(1.0+VMKI*ROXYD/(VOLWZ*FOSRH(K)))
      ELSE
      VMXD3=0.0
      ENDIF
...
C    2,FGOCP,FGOAP,CGOMD,RMOMT,RGN2F,ECHZ 
...
C    4,FOSRH(K),VOLX(L,NY,NX),ORGC(L,NY,NX),OSC(M,K,L,NY,NX)
