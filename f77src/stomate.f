...
C
C     DEHARDENING OF CONIFERS IN SPRING
C
      IF((IWTYP(NZ,NY,NX).EQ.1.OR.IWTYP(NZ,NY,NX).EQ.3)
     2.AND.IBTYP(NZ,NY,NX).GE.2)THEN
      FDBK(NB,NZ,NY,NX)=FDBK(NB,NZ,NY,NX)*AMAX1(0.0
     2,AMIN1(1.0,0.333*ATRP(NB,NZ,NY,NX)
     3/AMIN1(120.0,VRNL(NB,NZ,NY,NX))))
      ENDIF
C
