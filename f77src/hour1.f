...
C
      ...
      DIMENSION PSISK(100),TAUY(0:JC+1),RABSL(0:JC+1),RABPL(0:JC+1)
     2,RAFSL(0:JC+1),RAFPL(0:JC+1),IALBS(4,4),RADSL(JP,JY,JX)
     ...
     3,XVOLWC(0:2),ZL1(0:JZ,JY,JX),THETPZ(JZ,JY,JX),TRADC(JY,JX)
     4,TRAPC(JY,JX),TRADG(JY,JX),TRAPG(JY,JX),THETRX(0:2)
     ...
     4,ANH3X=0.07,AH2GX=0.14)
      ...
      PARAMETER (VISC=0.28E-12,BKDSX=1.89,ZW=0.005,CFW=0.5,FPSISR=-4.0
     2,FORGW=0.25E+06,PSIMA=-0.01E-03,DTHETW=1.0E-06
     3,THETPW=0.01,THETWP=1.0-THETPW)
...
C     IF(FHOL(L,NY,NX).LT.0.0)THEN
C     THETH=EXP((PSIMS(NY,NX)-LOG(-PSISE(L,NY,NX)-PSIMA))
C    2*PSD(L,NY,NX)/PSISD(NY,NX)+PSL(L,NY,NX))
C     FHOL(L,NY,NX)=POROS(L,NY,NX)-THETH
C     WRITE(*,3332)'FHOL',IYRC,L,FHOL(L,NY,NX),POROS(L,NY,NX)
C    2,THETH,FC(L,NY,NX),PSISE(L,NY,NX),PSIMA,-PSISE(L,NY,NX)-PSIMA
C     ENDIF
     ...
     3-VOLIH(L,NY,NX))
      EHUM(L,NY,NX)=0.200+0.200*AMIN1(0.5,CCLAY(L,NY,NX))
      EPOC(L,NY,NX)=1.0
...
C    2,THETF,FMPR(L,NY,NX)
      ENDIF
      WRITE(*,3333)'PPTYS',I,J,NX,NY,L,ISOIL(1,L,NY,NX)
     2,ISOIL(2,L,NY,NX),ISOIL(3,L,NY,NX),ISOIL(4,L,NY,NX)
     3,SCNV(L,NY,NX),SCNH(L,NY,NX),POROS(L,NY,NX),THETF
     2,FC(L,NY,NX),WP(L,NY,NX),BKDS(L,NY,NX),THW(L,NY,NX)
     3,VOLW(L,NY,NX),THI(L,NY,NX),THETI(L,NY,NX)
3333  FORMAT(A8,9I4,20E12.4)
C
C     HYDRAULIC CONDUCTIVITY FUNCTION FROM KSAT AND SOIL WATER RELEASE CURVE
C
C     IF(BKVL(L,NY,NX).GT.0.0)THEN
      SUM2=0.0
      DO 1320 K=1,100
      XK=K-1
...
C     DO 2340 N=1,3
C     IF(N.EQ.3)THEN
C     HCND(N,K,L,NY,NX)=SCNV(L,NY,NX)
C     ELSE
C     HCND(N,K,L,NY,NX)=SCNH(L,NY,NX)
C     ENDIF
2340  CONTINUE
2335  CONTINUE
C     ENDIF
...
C
     ...
     2*FMPR(L,NY,NX)+2.128*ROCK(L,NY,NX))
     3*AREA(3,L,NY,NX)*DLYR(3,L,NY,NX)
      ELSE
      STC(L,NY,NX)=0.0
      DTC(L,NY,NX)=0.0
      VHCM(L,NY,NX)=0.0
...
C
      ...
      FCR(NY,NX)=(-0.03/PSISE(0,NY,NX))**(1.0/FPSISR)
...
C
     ...
     2-0.012*ZS(NY,NX)*GSIN(NY,NX))*AREA(3,NU(NY,NX),NY,NX)
      VOLWG(NY,NX)=AMAX1(VOLWD(NY,NX),-DTBLX(NY,NX)
     ...
     2/BKVL(NU(NY,NX),NY,NX)
      ...
      ENDIF
      EHUM(0,NY,NX)=0.200+0.200*AMIN1(0.5,CCLAY(NU(NY,NX),NY,NX))
      EPOC(0,NY,NX)=0.150
C
...
C     DDLYR=(1.0-(VOLW(L,NY,NX)+VOLI(L,NY,NX))/VOLA(L,NY,NX))
C    2*DLYR(3,L,NY,NX)
C     IF(DLYR(3,L,NY,NX).GT.1.0E-03.OR.DDLYR.LT.0.0)THEN
C     DO 900 LL=NU(NY,NX),L
C     CDPTH(LL-1,NY,NX)=CDPTH(LL-1,NY,NX)+DDLYR
900   CONTINUE
C     DO 905 LL=NU(NY,NX),L
C     DLYR(3,LL,NY,NX)=(CDPTH(LL,NY,NX)-CDPTH(LL-1,NY,NX))
C     DPTH(LL,NY,NX)=0.5*(CDPTH(LL,NY,NX)+CDPTH(LL-1,NY,NX))
C     VOLT(L,NY,NX)=AREA(3,L,NY,NX)*DLYR(3,L,NY,NX)
C     VOLX(L,NY,NX)=VOLT(L,NY,NX)*FMPR(L,NY,NX)
C     VOLA(L,NY,NX)=POROS(L,NY,NX)*VOLX(L,NY,NX)
C     IF((I/30)*30.EQ.I.AND.J.EQ.15)THEN
C     WRITE(*,1114)'DDLYR',I,J,L,LL,DDLYR,VOLW(LL,NY,NX)
C    2,VOLI(LL,NY,NX),VOLA(LL,NY,NX),CDPTH(LL-1,NY,NX)
1114  FORMAT(A8,4I4,12E12.4)
C     ENDIF
905   CONTINUE
C     ENDIF
C     ENDIF
C     CDPTHZ(L,NY,NX)=CDPTH(L,NY,NX)-CDPTH(NU(NY,NX),NY,NX)
C    2+DLYR(3,NU(NY,NX),NY,NX)
C     IF(L.EQ.NU(NY,NX))THEN
C     DPTHZ(L,NY,NX)=0.5*CDPTHZ(L,NY,NX)
C     ELSE
C     DPTHZ(L,NY,NX)=0.5*(CDPTHZ(L,NY,NX)+CDPTHZ(L-1,NY,NX))
C     ENDIF
C
...
C
     ...
     2/VOLW(L,NY,NX))
      ...
      CION(L,NY,NX)=0.0
      ENDIF
C     IF(ZHY(L,NY,NX).GT.ZEROS(NY,NX)
C    2.AND.VOLW(L,NY,NX).GT.ZEROS(NY,NX))THEN
C     PH(L,NY,NX)=-LOG10(ZHY(L,NY,NX)/(VOLW(L,NY,NX)*1.0E+03))
C     ENDIF
      ENDIF
C
...
C     IF(BKVL(L,NY,NX).GT.0.0)THEN
     ...
     3/PSD(L,NY,NX))**SRP(L,NY,NX)*PSISD(NY,NX)))
      ...
      ENDIF
C     ELSE
C     PSISM(L,NY,NX)=PSISE(L,NY,NX)
C     ENDIF
C
C     SOIL OSMOTIC, GRAVIMETRIC AND MATRIC WATER POTENTIALS
C
      PSISO(L,NY,NX)=-8.3143E-06*TKS(L,NY,NX)*CION(L,NY,NX)
      PSISH(L,NY,NX)=0.0098*YDPTH(L,NY,NX)
      PSIST(L,NY,NX)=AMIN1(0.0,PSISM(L,NY,NX)+PSISO(L,NY,NX)
     2+PSISH(L,NY,NX))
C     IF((I/30)*30.EQ.I.AND.J.EQ.15)THEN
C     IF(I.GT.170.AND.NX.EQ.1.AND.NY.EQ.4)THEN
C     WRITE(*,1113)'PSISM1',I,J,NX,NY,L,PSISM(L,NY,NX),THETW(L,NY,NX)
...
C    5,DLYR(3,L,NY,NX),PSIST(L,NY,NX),PSISM(L,NY,NX),PSISO(L,NY,NX)
C    2,PSISH(L,NY,NX),TKS(L,NY,NX),CION(L,NY,NX)
1113  FORMAT(A8,5I4,30E12.4)
C     ENDIF
C
...
C
      VOLWRX(NY,NX)=THETRX(0)*RC0(0,NY,NX)+THETRX(1)*RC0(1,NY,NX)
     2+THETRX(2)*RC0(2,NY,NX)
      VOLR(NY,NX)=RC0(0,NY,NX)*1.0E-06/BKRS(0)
...
C
      ...
      DO 1135 L=1,JC
      IF(ZL(L-1,NY,NX).GE.DPTHS(NY,NX)-ZERO)THEN
      DO 1130 K=1,25
...
125   CONTINUE
      ENDIF
C
C     CANOPY AND GROUND SKY FRACTIONS
C
      FRADG(NY,NX)=1.0
...
C
     ...
     2.OR.ZNH3B(LFDPTH,NY,NX).GT.0.0).AND.IFNHB(NY,NX).EQ.0))THEN
      ...
      DPNHB(L,NY,NX)=AMAX1(0.025,FDPTH(I,NY,NX)-CDPTH(L-1,NY,NX))
...
2965  CONTINUE
      ENDIF
      IF(IYTYP(0,I,NY,NX).NE.0
     2.AND.(FERT(3,I,NY,NX).GT.0.0.OR.FERT(7,I,NY,NX).GT.0.0))THEN
      IF(IYTYP(0,I,NY,NX).EQ.1.OR.IYTYP(0,I,NY,NX).EQ.3)THEN
      IUTYP(NY,NX)=1
      ELSE
      IUTYP(NY,NX)=2
      ENDIF
