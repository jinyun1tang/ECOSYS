...
C
      ...
      XHS(NY,NX)=XNPH
      IF(BKDS(NU(NY,NX),NY,NX).GT.ZEROS(NY,NX))THEN
      XH(3,0,NY,NX)=XNPR
      ELSE
      XH(3,0,NY,NX)=1.0
      ENDIF
      DO 9975 L=NUI(NY,NX),NL(NY,NX)
      DO 9970 N=1,3
      IF(N.EQ.3)THEN
...
C     ENDIF
      ...
      IF(BKDX(L,NY,NX).GT.ZERO.AND.DLYR(3,L,NY,NX).GT.ZERO)THEN
      BKDS(L,NY,NX)=AMIN1(0.99*BKDSX,BKDX(L,NY,NX)
...
C    7,ZCA0P(L,NY,NX),ZCA0PB(L,NY,NX)
C     ENDIF
C
