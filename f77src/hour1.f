...
C
     ...
     2,FORGW=0.25E+06,HYGR=-2500.0,DTHETW=1.0E-06
...
C     POROS(L,NY,NX)=AMAX1(POROS(L,NY,NX),1.0-(BKDS(L,NY,NX)/PTDS))
C     ELSE
C     POROS(L,NY,NX)=1.0-(BKDS(L,NY,NX)/PTDS)
C     ENDIF
...
C
     ...
     2+0.36*CCLAY(L,NY,NX)+0.60E-06*CORGC(L,NY,NX)
      ...
      WRITE(*,3332)'FC',IYRC,L,FC(L,NY,NX),CCLAY(L,NY,NX),CORGC(L,NY,NX)
     2,CSAND(L,NY,NX)
3332  FORMAT(A8,2I4,12E12.4)
      ENDIF
     ...
     2+0.32E-06*CORGC(L,NY,NX)
      ...
      WRITE(*,3332)'WP',IYRC,L,WP(L,NY,NX),CCLAY(L,NY,NX),CORGC(L,NY,NX)
     2,FC(L,NY,NX)
      ENDIF
      ENDIF
...
C
     ...
     2*(PSL(L,NY,NX)-FCL(L,NY,NX))/PSISD(NY,NX)+PSL(L,NY,NX)))
      ...
      ENDIF
      WRITE(*,3332)'SCNV',IYRC,L,SCNV(L,NY,NX),POROS(L,NY,NX),THETF
     2,FMPR(L,NY,NX)
      ENDIF
     ...
     2*(PSL(L,NY,NX)-FCL(L,NY,NX))/PSISD(NY,NX)+PSL(L,NY,NX)))
      ...
      ENDIF
      WRITE(*,3332)'SCNH',IYRC,L,SCNH(L,NY,NX),POROS(L,NY,NX),THETF
     2,FMPR(L,NY,NX)
      ENDIF
...
C
      ...
      THETY(0,NY,NX)=(HYGR/PSISE(0,NY,NX))**(1.0/FPSISR)
...
C    2.AND.CDPTH(L-1,NY,NX).LT.DPTHA(NY,NX))THEN
      DDLYR=(1.0-(VOLW(L,NY,NX)+VOLI(L,NY,NX))/VOLA(L,NY,NX))
     2*DLYR(3,L,NY,NX)
...
900   CONTINUE
      ...
      DLYR(3,LL,NY,NX)=(CDPTH(LL,NY,NX)-CDPTH(LL-1,NY,NX))
