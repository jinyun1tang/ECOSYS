...
C
      DIMENSION RNHUI(2)
      PARAMETER (DPH2O=6.5E-09,SPALO=5.0E-21,SPFEO=6.3E-27
     2,SPCAC=5.0E-03,SPCAS=1.4E+01,SPALP=0.75E-15,SPFEP=0.75E-20
     3,SPCAM=7.0E+07,SPCAD=1.0E-01,SPCAH=6.4E-33,SXOH2=4.5E-05
     ...
     9,SHCAH2=SHCAH1/DPH2P**3,SYCAH2=SHCAH2*DPH2O**7)
      PARAMETER (MRXN=1,TPD=1.0E-03,TPDX=TPD/MRXN,TADA=3.3E-02
     2,TADAX=TADA/MRXN,TADC=3.3E-02,TADCX=TADC/MRXN
     3,TADC0=TADC*1.0E-02,TSL=0.5,TSLX=TSL/MRXN) 
      PARAMETER (DUKM=1.0,DUKI=2.5,A0=1.0,COOH=2.5E-02
     2,CCAMX=10.0)
      PARAMETER (SPNH4=1.0E-00,SPNH3=1.0E-00,SPNHU=1.25E-00
...
C    2,AOH1,R1,P1,P2,SP,SPX,RPALOX,RHAL1,RHALO1,RHALO2,RHALO3,RHALO4
C    3,AAL1*AOH1**3,SPALO 
C     ENDIF
C
C     IRON HYDROXIDE
C
...
C
      ...
      ENDIF
C     IF(I.EQ.180.AND.J.EQ.12)THEN
C     WRITE(*,1112)'APATITE',I,J,L,M,PCAPH1,ACA1 
...
C
      RXN4=TADCX*(XN4Q-XN41)*AN41/XN4Q 
      RXNB=TADCX*(XNBQ-XN4B)*AN4B/XNBQ 
C
C     H,AL,FE,CA,MG,NA,K EXCHANGE
C
      RXHY=TADCX*(XHYQ-XHY1)*AHY1/XHYQ 
      RXAL=TADCX*(XALQ-XAL1)*AALX/XALQ 
      RXFE=TADCX*(XFEQ-XFE1)*AFEX/XFEQ 
      RXCA=TADCX*(XCAQ-XCA1)*ACAX/XCAQ 
      RXMG=TADCX*(XMGQ-XMG1)*AMGX/XMGQ 
      RXNA=TADCX*(XNAQ-XNA1)*ANA1/XNAQ 
      RXKA=TADCX*(XKAQ-XKA1)*AKA1/XKAQ 
C     IF(I.EQ.180.AND.J.EQ.12)THEN
C     WRITE(*,1112)'RXFE',I,J,L,M,CCEC,XCAX,XN41,XHY1,XAL1,XFE1
C    2,XCA1,XMG1,XNA1,XKA1,AN41,AHY1,AALX,AFEX,ACAX,AMGX,ANA1
C    3,AKA1,RXN4,RXHY,RXAL,RXFE,RXCA,RXMG,RXNA,RXKA 
C     ENDIF
      ELSE
      RXN4=0.0
      RXNB=0.0
...
C
      ...
      COH1=COH1+RHOH
C     IF(I.EQ.180.AND.J.EQ.12)THEN
C     WRITE(*,1111)'CCA1',I,J,L,M,ACA1,AHY1,AH1P1,AH2P1,ACO31,AHCO31 
...
C
      TRCO2(L,NY,NX)=TRCO3(L,NY,NX)+2.0*TRHCO(L,NY,NX)
     2+TRCAC(L,NY,NX)+TRMGC(L,NY,NX)
     2+TRNAC(L,NY,NX)+2.0*TRCAH(L,NY,NX)
     2+2.0*TRMGH(L,NY,NX)+TRCACO(L,NY,NX)
...
C     WRITE(20,1111)'TBION',I,J,L,M,TBION(L,NY,NX)
C
...
C     NH4 EXCHANGE IN NON-BAND AND BAND SOIL ZONES
C
      RXN4=TADC*(XN4Q-XN41)*CN41/XN4Q 
      RXNB=TADC*(XNBQ-XN4B)*CN4B/XNBQ 
C     IF(J.EQ.12.AND.L.EQ.0)THEN
C     WRITE(*,2222)'RXN4',I,J,L,RXN4,CN41,XN41,CCAX,CCA1,XCAQ,CCEC
C    2,FN4X,FCAQ,GKC4(L,NY,NX),PH(L,NY,NX),VOLWBK
...
C    WRITE(20,2222)'RNH4',I,J,L,RNH4,CHY1,CN31,DPN4,CN41
...
C     IF(L.EQ.1)THEN
C     WRITE(20,24)'RHP1',I,J,L,RHP1,RH2P,RXH1P
C    2,TRX1P(L,NY,NX),TRH2P(L,NY,NX)
C     WRITE(20,24)'RHP2',I,J,L,RHP2,RH2P,RXH2P,RYH2P
...
C     IF(L.EQ.1)THEN
C     WRITE(20,9984)'TRN3S',I,J,L,TRN3S(L,NY,NX),TRN3B(L,NY,NX)
C    2,RSNUAA,RSNUBA,RSNUBB
...
C
      IF(VOLWM(NPH,0,NY,NX).GT.ZEROS(NY,NX))THEN
      CCEC0=AMAX1(0.0,COOH*ORGC(0,NY,NX)/BKVLX)
      ELSE
      CCEC0=0.0
      ENDIF
      CALX=AMAX1(ZERO,CAL1)**0.333
...
C
      ...
      RNH4=(CHY1*CN31-DPN4*CN41)/(DPN4+CHY1)
C     IF(J.EQ.12)THEN
C     WRITE(20,2223)'RXN4',I,J,RXN4,CN41,XN41,CCAX,CCA1,CCO20,CCO31 
