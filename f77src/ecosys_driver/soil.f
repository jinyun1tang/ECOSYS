...
C     ARGUMENTS IN SUBROUTINE CALLS ARE FROM MAIN
C
...
C
C     READ INPUT DATA FOR SITE, TOPOGRAPHY AND SOILS IN READI.F 
C     AND SOIL MANAGEMENT IN 'READS.F', 
C     AND SET UP OUTPUT AND CHECKPOINT FILES IN 'FOUTS.F'
C
...
C     IN 'ROUTS' IF RUN IS RESUMED FROM AN EARLIER RUN AS INDICATED 
C     IN THE OPTIONS FILE
C
C     if DATA(20)= YES in line 6 of the options file,
C        then resume from earlier run that started on the date 
C        in line 3 of the options file 
C     IDAYR,IYRR=day, year of resumption indicated in  line 1 
C        of the options file
C
...
C     WRITE(*,333)'ROUTS'
      ...
      ENDIF
C
C     RECOVER PLANT FUNCTIONAL TYPES FROM EARLIER RUN IN 'ROUTQ'
C
C     WRITE(*,333)'ROUTQ'
      CALL ROUTQ(NT,NE,NAX,NDX,NTX,NEX,NHW,NHE,NVN,NVS)
C
C     READ INPUT DATA FOR PLANT FUNCTIONAL TYPES AND MANAGEMENT 
...
C     IN 'ROUTP' IF RUN IS RESUMED FROM AN EARLIER RUN AS INDICATED 
C     IN THE OPTIONS FILE 
C
C     if DATA(20)= YES in line 6 of the options file,
C        then resume from earlier run that started on the date 
...
C     WRITE(*,333)'ROUTP'
      ...
      ENDIF
C
C     INITIALIZE SOLUTE CHEMISTRY VARIABLES IN 'STARTE'
C     FOR SOIL, SURFACE LITTER, PRECIPITATION AND IRRIGATION
C     FROM INPUTS IN SOIL, WEATHER AND IRRGATION FILES READ IN READI
C     AND READS. 
C
      IF((DATA(20).EQ.'YES'.AND.IGO.EQ.0).OR.IDAYR.NE.IOLD)THEN
C     WRITE(*,333)'STARTE'
...
C     IF END OF SCENE
C
C     IFIN=last day of scene from options file
C
...
C
      DO 9995 J=1,24
C
C     CALCULATE HOURLY WEATHER VARIABLES USED IN SUBROUTINES BELOW 
C     IN 'WTHR' FROM INPUTS IN WEATHER AND OPTIONS FILES
C
C     WRITE(*,333)'WTHR'
...
C
      DO 9990 NFZ=1,NFH
C
C     CALCULATE SUBHOURLY VARIABLES IN 'HOUR1'
C
C     WRITE(*,333)'HOUR1'
333   FORMAT(A8)
      CALL HOUR1(I,J,NFZ,NHW,NHE,NVN,NVS)
C
C     CALCULATE SNOW, LITTER AND SOIL SURFACE ENERGY BALANCES, 
C     LATERAL AND VERTICAL WATER AND HEAT FLUXES IN 'WATSUB'
C
C     WRITE(*,333)'WAT'
...
C
C     CALCULATE SOLUTE PRECIPITATION-DISSOLUTION, ADSORPTION-DESORPTION
C     AND ION PAIRING EQUILIBRIA IN 'SOLUTE'
C
C     WRITE(*,333)'SOL'
      CALL SOLUTE(I,J,NFZ,NHW,NHE,NVN,NVS)
C
C     CALCULATE GAS AND SOLUTE FLUXES IN 'TRNSFR' FROM WATER FLUXES
C     IN WATSUB AND DIFFUSION COEFFICIENTS IN HOUR1
C
C     WRITE(*,333)'TRN'
...
C
C     CALCULATE SOIL SEDIMENT TRANSPORT IN 'EROSION' FROM WATER FLUXES
C     IN WATSUB IF EROSION OPTION SELECTED IN SITE FILE 
C
C     WRITE(*,333)'ERO'
      CALL EROSION(I,J,NFZ,NHW,NHE,NVN,NVS)
C
C     AGGREGATED CANOPY AND ROOT C AND NUTRIENT EXCHANGE FOR ALL PLANT
C     SPECIES IN 'EXTRACT' FROM THOSE FOR EACH PLANT SPECIES 
C     IN UPTAKE AND GROSUB
C
C     WRITE(*,333)'EXTR'
      CALL EXTRACT(I,J,NFZ,NHW,NHE,NVN,NVS)
C
C     UPDATE ALL SOIL STATE VARIABLES FOR WATER, HEAT, GAS, SOLUTE
C     AND SEDIMENT FLUXES IN 'REDIST' FROM FLUXES CALCULATED IN
C     WATSUB,NITRO, SOLUTE, TRNSFR, TRNSFRS, EROSION 
C     AND EXTRACT
C
C     WRITE(*,333)'RED'
...
9990  CONTINUE
C
C     WRITE HOURLY SOIL AND PLANT OUTPUT FOR ALL GRID CELLS 
C     IN 'OUTSH' AND 'OUTPH' FROM SELECTIONS MADE IN OUTPUT EDITOR
C     FILES IN SCENE RUNSCRIPT AT FREQUENCY SELECTED IN LINE 11 
C     OF OPTION FILE
C
...
C     WRITE(*,333)'OUTSH'
      ...
      ENDIF
C
C     WRITE OUTPUT FOR SPECIAL PROJECTS (EG MIPS) IN VISUAL 
C     IF SELECTED IN DATA(18) FROM LINE 4 OF OPTIONS FILE
C
...
C     AT FREQUENCY SELECTED IN LINE 11 OF OPTION FILE
C     AND AT THE END OF EACH SCENE
C
...
C     WRITE(*,333)'WOUTP'
      ...
      ENDIF
C
C     WRITE DAILY SOIL AND PLANT OUTPUT FOR ALL GRID CELLS IN 'OUTSD'
C     AND 'OUTPD' FROM SELECTIONS MADE IN OUTPUT EDITOR FILES IN SCENE
C     RUNSCRIPT AT FREQUENCY SELECTED IN LINE 11 OF OPTION FILE
C
...
C     WRITE(*,333)'OUTPD'
      ...
      ENDIF
C
C     PERFORM BALANCE CHECKS IN 'EXEC' FOR WATER, HEAT, OXYGEN,
C     C, N, P AND IONS
C
C     WRITE(*,333)'EXEC'
      CALL EXEC(I)
C
C     RE-INITIALIZE MODEL WITH ALL SOIL AND PLANT STATE VARIABLES 
C     AND OTHER INFORMATION BY READING CHECKPOINT FILES IN ROUTS 
C     AND ROUTP FROM THOSE WRITTEN IN WOUTS AND WOUTP
C     FROM CONDITIONS SET IN EXEC
C
...
C     FROM OUTPUT FILES FOR ALL GRID CELLS WRITTEN IN 
C     OUTSH, OUTSD, OUTPH, AND OUTPD
C
Cifdef _WIN_
