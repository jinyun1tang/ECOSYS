...
9999  CONTINUE
C
C     APPEND START YEAR TO CHECKPOINT FILE NAMES AND OPEN
C
C     DATA(20)=’YES’:resume from earlier run that started from year in
C        IDATA(9)
C             =’NO’:start new run starting from year in IDATA(3)
C
...
C
C     MATCH PREVIOUS AND CURRENT PLANT SPECIES DATAP AND MANAGEMENTS
C     DATAM INTO COMMON ARRAY INDICATOR NZ FOR CONTINUITY 
C     DURING MODEL RUN
C
      DO 7975 NX=NHW,NHE
      DO 7970 NY=NVN,NVS
...
200   CONTINUE
C
C     ADD ANY NEW PLANT SPECIES FILES DATAP, PLANT MANAGEMENT 
C     FILES DATAM TO EXISTING FILES
C
