      ...
      SUBROUTINE hfunc(I,J,NFZ,NHW,NHE,NVN,NVS)
C
...
C
     ...
     2.AND.IDTH(NZ,NY,NX).EQ.1)THEN
      ...
      IDTH(NZ,NY,NX)=0
      IFLGN(NZ,NY,NX)=1
      IF(NFZ.EQ.1)CALL STARTQ(NX,NX,NY,NY,NZ,NZ)
...
4444  FORMAT(A8,6I8,A16,10I8,1E12.4)
C
...
C
      ...
      IF(WTRTL(N,L,NZ,NY,NX).GT.ZEROP2(NZ,NY,NX))THEN
      CCPOLR(N,L,NZ,NY,NX)=AMAX1(0.0,CPOOLR(N,L,NZ,NY,NX)
     2/(WTRTL(N,L,NZ,NY,NX)+CPOOLR(N,L,NZ,NY,NX)))
...
C
     ...
     2/(WTLS(NZ,NY,NX)+CPOOLP(NZ,NY,NX)))
      CCPLNP(NZ,NY,NX)=AMAX1(0.0,CPOLNP(NZ,NY,NX)
     2/(WTLS(NZ,NY,NX)+CPOLNP(NZ,NY,NX)))
      CZPOLP(NZ,NY,NX)=AMAX1(0.0,ZPOOLP(NZ,NY,NX)
     ...
     2/(WTLSB(NB,NZ,NY,NX)+CPOOL(NB,NZ,NY,NX)))
      CZPOLB(NB,NZ,NY,NX)=AMAX1(0.0,ZPOOL(NB,NZ,NY,NX)
     2/(WTLSB(NB,NZ,NY,NX)+ZPOOL(NB,NZ,NY,NX)))
      CPPOLB(NB,NZ,NY,NX)=AMAX1(0.0,PPOOL(NB,NZ,NY,NX)
     2/(WTLSB(NB,NZ,NY,NX)+PPOOL(NB,NZ,NY,NX)))
      ELSE
...
C    3,ARLSP,RTDP1(1,1,NZ,NY,NX),WTRVC(NZ,NY,NX),PPX(NZ,NY,NX)
...
C    7,FNOD(NZ,NY,NX),XTLI(NZ,NY,NX),PPZ(NZ,NY,NX)
C    8,PPX(NZ,NY,NX),PP(NZ,NY,NX)
224   FORMAT(A8,12I6,30E12.4)
C     ENDIF
      IF(IFLGI(NZ,NY,NX).EQ.0.AND.IFLGC(NZ,NY,NX).EQ.1)THEN
      IF(PP(NZ,NY,NX).GT.0.0)THEN
      IF(PSIRG(1,NG(NZ,NY,NX),NZ,NY,NX).GT.PSILM)THEN
      IF(ISTYP(NZ,NY,NX).NE.0
     2.OR.(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1)
     3.OR.IDAY(2,NB1(NZ,NY,NX),NZ,NY,NX).EQ.0)THEN
     ...
     3+NNOD(NZ,NY,NX)/FNOD(NZ,NY,NX)+XTLI(NZ,NY,NX)
     4.OR.(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1))THEN
...
C
C     IDAY(1,=emergence date (0=not emerged)
C     IFLGI=PFT initialization flag:0=no,1=yes
C     IDTHB=branch life flag:0=living,1=dead
...
C    2,IWTYP(NZ,NY,NX),ISTYP(NZ,NY,NX),IWTYP(NZ,NY,NX)
C    3,VRNF(NB,NZ,NY,NX),VRNX(NB,NZ,NY,NX)
C    2,PSTG(NB,NZ,NY,NX),GROUP(NB,NZ,NY,NX),PSTGI(NB,NZ,NY,NX)
4648  FORMAT(A8,9I4,20E12.4)
C     ENDIF
      IF(IWTYP(NZ,NY,NX).EQ.0
     2.OR.VRNF(NB,NZ,NY,NX).LT.VRNX(NB,NZ,NY,NX)
     3.OR.(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1))THEN
      IF(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1
     2.AND.PSTG(NB,NZ,NY,NX).GT.GROUP(NB,NZ,NY,NX)+PSTGI(NB,NZ,NY,NX)
     3.AND.IDAY(2,NB,NZ,NY,NX).EQ.0)THEN
      RNI=0.0
      RLA=0.0
      ELSE
      ...
      RLA=AMAX1(0.0,XRLA(NZ,NY,NX)*TFNP*XNFH)
      ENDIF
C
C     NODE INITIATION AND LEAF APPEARANCE RATES SLOWED 
...
C    2,IDAY0(NZ,NY,NX),IYR0(NZ,NY,NX),IPTYP(NZ,NY,NX)
...
C     ENDIF
     ...
     3.AND.(ISTYP(NZ,NY,NX).NE.0.OR.IWTYP(NZ,NY,NX).NE.1
     3.OR.VRNF(NB,NZ,NY,NX).GT.VRNX(NB,NZ,NY,NX)))
     3.OR.(((ISTYP(NZ,NY,NX).EQ.1.AND.(IWTYP(NZ,NY,NX).EQ.1
     4.OR.IWTYP(NZ,NY,NX).EQ.3))
     5.OR.(ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.0))
     6.AND.ZC(NZ,NY,NX).GE.DPTHS(NY,NX)-ZERO
...
C     STEM ELONGATION
C
C     IDAY(3,=date of start of stem elongation and setting max seed
C        number 
C     GSTGI=vegetative node number normalized for maturity group
...
C
     ...
     7.AND.VRNF(NB,NZ,NY,NX).GT.VRNX(NB,NZ,NY,NX))THEN
      ...
      ENDIF
C
C     IDAY(4,=date of mid stem elongation
...
C
     ...
     7.AND.VRNF(NB,NZ,NY,NX).GT.VRNX(NB,NZ,NY,NX))THEN
      ...
      ENDIF
C
C     IDAY(5,=date of end of stem elongation and setting max seed
C        number
C     ISTYP,IWTYP,IPTYP=growth habit,phenology,photoperiod type 
C        from PFT file
C     GSTGI=vegetative node number normalized for maturity group
...
C
C     IDAY(6,=date of start of anthesis and setting final seed number
...
C
C     IDAY(7,=date of start of grain filling and setting max seed size
C     GSTGF=reproductive node number normalized for maturity group
C     GSTGR=normalized growth stage durations for reproductive
C        phenology
C     ISTYP,IWTYP,IPTYP=growth habit,phenology,photoperiod type 
C        from PFT file
C     DYLX,DLYN=daylength of previous,current day
...
C
C     IDAY(9,=end date of setting max seed size
C     GSTGF=reproductive node number normalized for maturity group
C     GSTGR=normalized growth stage durations for reproductive
C        phenology
C
...
C   
      ...
      VRNY(NB,NZ,NY,NX)=VRNY(NB,NZ,NY,NX)+1.0*XNFH
      VRNZ(NB,NZ,NY,NX)=0.0
      ELSE
      VRNY(NB,NZ,NY,NX)=0.0
      VRNZ(NB,NZ,NY,NX)=VRNZ(NB,NZ,NY,NX)+1.0*XNFH
      ENDIF
C
...
C
     ...
     4.AND.IFLGE(NB,NZ,NY,NX).EQ.0)THEN
      ...
      VRNS(NB,NZ,NY,NX)=AMAX1(0.0,VRNS(NB,NZ,NY,NX)-1.0*XNFH)
      ENDIF
      ENDIF
      IF((VRNS(NB,NZ,NY,NX).GE.VRNL(NB,NZ,NY,NX)
     2.AND.(ISTYP(NZ,NY,NX).NE.0.OR.IWTYP(NZ,NY,NX).NE.1))
C    3.OR.IDAY(2,NB,NZ,NY,NX).NE.0)
     2.OR.(ALAT(NY,NX).GT.0.0.AND.I.EQ.173)
     3.OR.(ALAT(NY,NX).LT.0.0.AND.I.EQ.355))THEN
      VRNF(NB,NZ,NY,NX)=0.0
      ENDIF
...
C
     ...
     2.AND.IDAY(2,NB,NZ,NY,NX).NE.0)
     3.OR.(DYLN(NY,NX).GT.DYLX(NY,NX)
     3.AND.ISTYP(NZ,NY,NX).EQ.0.AND.IWTYP(NZ,NY,NX).EQ.1))THEN
     ...
     2.AND.IFLGE(NB,NZ,NY,NX).EQ.1)THEN
      ...
      ENDIF
C     IF(NZ.EQ.3)THEN
C     WRITE(*,4646)'VRNS',I,J,NFZ,NZ,NB,IDAY(2,NB,NZ,NY,NX)
...
C
     ...
     2.OR.IWTYP(NZ,NY,NX).EQ.5)THEN
      ...
      VRNS(NB,NZ,NY,NX)=AMAX1(0.0,VRNS(NB,NZ,NY,NX)-12.0*XNFH)
      ENDIF
      ENDIF
...
C
      ...
      IF((DYLN(NY,NX).GE.DYLX(NY,NX).OR.DYLN(NY,NX).GE
     2.DYLM(NY,NX)-2.0).AND.IFLGE(NB,NZ,NY,NX).EQ.0)THEN
