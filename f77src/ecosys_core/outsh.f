...
C
      ...
      IF(CHOICE(K,N-20).EQ.'YES')THEN
      M=M+1
      IF(K.EQ.1)HEAD(M)=TEVPGH(NY,NX)*1000.0/AREA(3,NU(NY,NX),NY,NX)
      ...
      IF(K.EQ.6)HEAD(M)=AMAX1(0.0,(VOLSS(NY,NX)+VOLIS(NY,NX)
...
C
     ...
     2/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.6)HEAD(M)=TRNS(NY,NX)*277.8/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.7)HEAD(M)=TLES(NY,NX)*277.8/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.8)HEAD(M)=TSHS(NY,NX)*277.8/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.9)HEAD(M)=TGHS(NY,NX)*277.8/AREA(3,NU(NY,NX),NY,NX)
      ...
      IF(K.EQ.36)HEAD(M)=TCQ(NY,NX)
      IF(K.EQ.37)HEAD(M)=VPQ(NY,NX)*TKQ(NY,NX)/2.173E-03 
      ENDIF
1025  CONTINUE
      WRITE(LUN,'(A16,F8.3,4X,A8,I8,50E16.7E3)')OUTFILS(N-20,NY,NX)
     2,DOY,CDATE,J,(HEAD(K),K=1,M)
