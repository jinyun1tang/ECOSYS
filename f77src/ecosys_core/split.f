...
1100  READ(LUN,'(A16,F8.3,4X,A8,50E16.7)',END=1105)OUTFILE
     2,DOY,CDATE,(HEAX(K),K=1,NOUTS(N-20))
      IF(OUTFILE.EQ.OUTFILS(N-20,NY,NX))THEN
      WRITE(LUX,'(F8.3,4X,A8,50E16.7)')
...
1110  CONTINUE
      ...
      LUX=N+40
      REWIND(LUN)
      OPEN(LUX,FILE=trim(trim(outdir)//OUTFILP(N-20,NZ,NY,NX))//".txt"
     2,STATUS='UNKNOWN')
      READ(LUN,'(A12,2A8,50A16)')CDOY,DATE,HOUR
     2,(HEAD(K),K=1,NOUTP(N-20))
      WRITE(LUX,'(A12,2A8,50A16)')CDOY,DATE,HOUR
     2,(HEAD(K),K=1,NOUTP(N-20))
1200  READ(LUN,'(A16,F8.3,4X,A8,I8,50E16.7)',END=1205)OUTFILE
...
1210  CONTINUE
     ...
     2,STATUS='UNKNOWN')
      ...
      IF(N.LE.29)THEN
1300  READ(LUN,'(A16,F8.3,4X,A8,50E16.7)',END=1305)OUTFILE
     ...
     2DOY,CDATE,(HEAX(K),K=1,NOUTP(N-20))
      ...
      GO TO 1300
      ELSE
1400  READ(LUN,'(A16,F8.3,4X,A8,A16,I16,50E16.7)',END=1305)OUTFILE
     2,DOY,CDATE,CHEAD,IHEAD,(HEAX(K),K=1,NOUTP(N-20))
      IF(OUTFILE.EQ.OUTFILP(N-20,NZ,NY,NX))THEN
      WRITE(LUX,'(F8.3,4X,A8,A16,I16,50E16.7)')
