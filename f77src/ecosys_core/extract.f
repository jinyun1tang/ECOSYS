...
C     FCHGC=fraction of anaerobic C combustion to CO2
...
C
      IF(RCGCK(NY,NX).GT.ZEROS(NY,NX))THEN
      ROGCK=RCGCK(NY,NX)*2.667
      ROGOK=AMIN1(ROGCK*OXYQ(NY,NX)/(OXYQ(NY,NX)+COXYGK),OXYC(NY,NX))
      RCGOK=ROGOK/2.667
      RCHOK=(RCGCK(NY,NX)-RCGOK)*FCHGC
     ...
     2*OXYQ(NY,NX)/(OXYQ(NY,NX)+COXYGK)
      ...
      FCBOX=AMIN1(1.0,RCGOK/RCGCK(NY,NX))
      FCBCH=AMIN1(1.0,RCHOK/(RCGCK(NY,NX)*FCHGC))
      HCBFC=RCGOK*GCBCO+RCHOK/FCHGC*GCBCH+RC4OK*GCBC4 
      ELSE
      ROGOK=0.0
      RCGOK=0.0
      RCHOK=0.0
...
C     VCOXF=canopy+standing dead C loss from fire
C
      IF(RCGCKZ(NZ,NY,NX).GT.ZEROP(NZ,NY,NX))THEN
      HCBFCZ(NZ,NY,NX)=HCBFC*RCGCKZ(NZ,NY,NX)
     2/(RCGCKZ(NZ,NY,NX)+RCGDKZ(NZ,NY,NX))
     ...
     4+RWTGRBP(NB,NZ,NY,NX)+RPPOLNB(NB,NZ,NY,NX)+RWTNDBP(NB,NZ,NY,NX)
      ZNH4S(0,NY,NX)=ZNH4S(0,NY,NX)+RCMBN*FCBOX*FCOMN     
      H2PO4(0,NY,NX)=H2PO4(0,NY,NX)+RCMBP*FCBOX*FCOMP
      WTSTDG(5,NZ,NY,NX)=WTSTDG(5,NZ,NY,NX)+RCMBC*FCBCH*FCHMC
      VCOXF(NZ,NY,NX)=VCOXF(NZ,NY,NX)+RCMBC*FCBCH*FCHMC 
      ZNH4S(0,NY,NX)=ZNH4S(0,NY,NX)+RCMBN*FCBCH*FCHMN 
      H2PO4(0,NY,NX)=H2PO4(0,NY,NX)+RCMBP*FCBCH*FCHMP 
      COX=COX+RCMBC*FCBOX*FCOGC
      ZOX=ZOX+RCMBN*FCBOX*FCOGN 
      POX=POX+RCMBP*FCBOX*FCOGP 
      CHX=CHX+RCMBC*FCBCH*FCHGC
      ZOX=ZOX+RCMBN*FCBCH*FCHGN
      POX=POX+RCMBP*FCBCH*FCHGP
      Z4M=Z4M+RCMBN*FCBOX*FCOMN 
      P4M=P4M+RCMBP*FCBOX*FCOMP 
      Z4M=Z4M+RCMBN*FCBCH*FCHMN 
      P4M=P4M+RCMBP*FCBCH*FCHMP 
8750  CONTINUE
      ENDIF
C
...
C     VCOXF=fire C loss
C
      IF(RCGDKZ(NZ,NY,NX).GT.ZEROP(NZ,NY,NX))THEN
      HCBFDZ(NZ,NY,NX)=HCBFC*RCGDKZ(NZ,NY,NX)
     2/(RCGCKZ(NZ,NY,NX)+RCGDKZ(NZ,NY,NX))
      DO 8740 M=1,4
      ZNH4S(0,NY,NX)=ZNH4S(0,NY,NX)+RWTSTDN(M,NZ,NY,NX)*FCBOX*FCOMN     
      H2PO4(0,NY,NX)=H2PO4(0,NY,NX)+RWTSTDP(M,NZ,NY,NX)*FCBOX*FCOMP
      WTSTDG(5,NZ,NY,NX)=WTSTDG(5,NZ,NY,NX)+RWTSTDG(M,NZ,NY,NX)
     2*FCBCH*FCHMC 
      VCOXF(NZ,NY,NX)=VCOXF(NZ,NY,NX)+RWTSTDG(M,NZ,NY,NX)*FCBCH*FCHMC 
      ZNH4S(0,NY,NX)=ZNH4S(0,NY,NX)+RWTSTDN(M,NZ,NY,NX)*FCBCH*FCHMN 
      H2PO4(0,NY,NX)=H2PO4(0,NY,NX)+RWTSTDP(M,NZ,NY,NX)*FCBCH*FCHMP 
      COX=COX+RWTSTDG(M,NZ,NY,NX)*FCBOX*FCOGC
      ZOX=ZOX+RWTSTDN(M,NZ,NY,NX)*FCBOX*FCOGN
      POX=POX+RWTSTDP(M,NZ,NY,NX)*FCBOX*FCOGP 
      Z4M=Z4M+RWTSTDN(M,NZ,NY,NX)*FCBOX*FCOMN 
      P4M=P4M+RWTSTDP(M,NZ,NY,NX)*FCBOX*FCOMP
      CHX=CHX+RWTSTDG(M,NZ,NY,NX)*FCBCH*FCHGC
      ZOX=ZOX+RWTSTDN(M,NZ,NY,NX)*FCBCH*FCHGN
      POX=POX+RWTSTDP(M,NZ,NY,NX)*FCBCH*FCHGP
      Z4M=Z4M+RWTSTDN(M,NZ,NY,NX)*FCBCH*FCHMN 
      P4M=P4M+RWTSTDP(M,NZ,NY,NX)*FCBCH*FCHMP
8740  CONTINUE
C
...
C
      ZNH4S(0,NY,NX)=ZNH4S(0,NY,NX)+RWTSTDN(5,NZ,NY,NX)
     2*FCBOX*FCOMN     
      H2PO4(0,NY,NX)=H2PO4(0,NY,NX)+RWTSTDP(5,NZ,NY,NX)
     2*FCBOX*FCOMP
      ZNH4S(0,NY,NX)=ZNH4S(0,NY,NX)+RWTSTDN(5,NZ,NY,NX)
     2*FCBCH*FCHMN 
      H2PO4(0,NY,NX)=H2PO4(0,NY,NX)+RWTSTDP(5,NZ,NY,NX)
     2*FCBCH*FCHMP 
      COX=COX+RWTSTDG(5,NZ,NY,NX)*FCBOX 
      ZOX=ZOX+RWTSTDN(5,NZ,NY,NX)*FCBOX*FCOGN     
      POX=POX+RWTSTDP(5,NZ,NY,NX)*FCBOX*FCOGP
      Z4M=Z4M+RWTSTDN(5,NZ,NY,NX)*FCBOX*FCOMN 
      P4M=P4M+RWTSTDP(5,NZ,NY,NX)*FCBOX*FCOMP
      CHX=CHX+RWTSTDG(5,NZ,NY,NX)*FCBCH
      ZOX=ZOX+RWTSTDN(5,NZ,NY,NX)*FCBCH*FCHGN
      POX=POX+RWTSTDP(5,NZ,NY,NX)*FCBCH*FCHGP
      Z4M=Z4M+RWTSTDN(5,NZ,NY,NX)*FCBCH*FCHMN 
      P4M=P4M+RWTSTDP(5,NZ,NY,NX)*FCBCH*FCHMP
C     TLNH4=TLNH4+Z4M
...
C    2,FCBOX,FCBCH,FCBOX+FCBCH,COX,CHX,ZOX,Z4M,POX,P4M
C    3,VCO2R(NY,NX),VCH4R(NY,NX),VNOXR(NY,NX),VPOXR(NY,NX)
C    4,VCOXF(NZ,NY,NX),VNOXF(NZ,NY,NX),VPOXF(NZ,NY,NX)
C    5,RCGCK(NY,NX),RC4CK(NY,NX),RCGCKZ(NZ,NY,NX),RCGDKZ(NZ,NY,NX)
C    6,OXYC(NY,NX),OXYQ(NY,NX),CH4Q(NY,NX)
5567  FORMAT(A8,6I4,30F14.6)
C     ENDIF
      ENDIF
C
C     COMBUST ROOT
...
C
      ...
      IF(RCGSKR(L,NZ,NY,NX).GT.ZEROS(NY,NX))THEN
      FCBOXR=AMIN1(1.0,RCGOX(L,NY,NX)/RCGSK(L,NY,NX))
      FCBCHR=AMIN1(1.0,RCHOX(L,NY,NX)/(RCGSK(L,NY,NX)*FCHGC))
      IF(L.EQ.NU(NY,NX)+1)THEN
C     CO2G(L,NY,NX)=CO2G(L,NY,NX)+RWTRVC(NZ,NY,NX)
C    2*FCBOXR*FCOGC
      ZNH4S(L,NY,NX)=ZNH4S(L,NY,NX)+RWTRVN(NZ,NY,NX)
     2*FCBOXR*FCOMN     
      H2PO4(L,NY,NX)=H2PO4(L,NY,NX)+RWTRVP(NZ,NY,NX)
     2*FCBOXR*FCOMP
C     CH4G(L,NY,NX)=CH4G(L,NY,NX)+RWTRVC(NZ,NY,NX)
C    2*FCBCHR*FCHGC
      OSC(5,1,L,NY,NX)=OSC(5,1,L,NY,NX)+RWTRVC(NZ,NY,NX)
     2*FCBCHR*FCHMC 
      ZNH4S(L,NY,NX)=ZNH4S(L,NY,NX)+RWTRVN(NZ,NY,NX)
     2*FCBCHR*FCHMN 
      H2PO4(L,NY,NX)=H2PO4(L,NY,NX)+RWTRVP(NZ,NY,NX)
     2*FCBCHR*FCHMP
      COX=COX+RWTRVC(NZ,NY,NX)*FCBOXR*FCOGC 
      ZOX=ZOX+RWTRVN(NZ,NY,NX)*FCBOXR*FCOGN 
      POX=POX+RWTRVP(NZ,NY,NX)*FCBOXR*FCOGP
      Z4M=Z4M+RWTRVN(NZ,NY,NX)*FCBOXR*FCOMN 
      P4M=P4M+RWTRVP(NZ,NY,NX)*FCBOXR*FCOMP
      CHX=CHX+RWTRVC(NZ,NY,NX)*FCBCHR*FCHGC
      ZOX=ZOX+RWTRVN(NZ,NY,NX)*FCBCHR*FCHGN
      POX=POX+RWTRVP(NZ,NY,NX)*FCBCHR*FCHGP
      COR=COR+RWTRVC(NZ,NY,NX)*FCBCHR*FCHMC 
      Z4M=Z4M+RWTRVN(NZ,NY,NX)*FCBCHR*FCHMN 
      P4M=P4M+RWTRVP(NZ,NY,NX)*FCBCHR*FCHMP
      ENDIF
      DO 8885 N=1,MY(NZ,NY,NX)
C     CO2G(L,NY,NX)=CO2G(L,NY,NX)+RCPOOLR(N,L,NZ,NY,NX)
C    2*FCBOXR*FCOGC
      ZNH4S(L,NY,NX)=ZNH4S(L,NY,NX)+RZPOOLR(N,L,NZ,NY,NX)
     2*FCBOXR*FCOMN     
      H2PO4(L,NY,NX)=H2PO4(L,NY,NX)+RPPOOLR(N,L,NZ,NY,NX)
     2*FCBOXR*FCOMP
C     CH4G(L,NY,NX)=CH4G(L,NY,NX)+RCPOOLR(N,L,NZ,NY,NX)
C    2*FCBCHR*FCHGC
      OSC(5,1,L,NY,NX)=OSC(5,1,L,NY,NX)+RCPOOLR(N,L,NZ,NY,NX)
     2*FCBCHR*FCHMC
      ZNH4S(L,NY,NX)=ZNH4S(L,NY,NX)+RZPOOLR(N,L,NZ,NY,NX)
     2*FCBCHR*FCHMN 
      H2PO4(L,NY,NX)=H2PO4(L,NY,NX)+RPPOOLR(N,L,NZ,NY,NX)
     2*FCBCHR*FCHMP
      COX=COX+RCPOOLR(N,L,NZ,NY,NX)*FCBOXR*FCOGC     
      ZOX=ZOX+RZPOOLR(N,L,NZ,NY,NX)*FCBOXR*FCOGN     
      POX=POX+RPPOOLR(N,L,NZ,NY,NX)*FCBOXR*FCOGP
      CHX=CHX+RCPOOLR(N,L,NZ,NY,NX)*FCBCHR*FCHGC
      ZOX=ZOX+RZPOOLR(N,L,NZ,NY,NX)*FCBCHR*FCHGN
      POX=POX+RPPOOLR(N,L,NZ,NY,NX)*FCBCHR*FCHGP
      Z4M=Z4M+RZPOOLR(N,L,NZ,NY,NX)*FCBOXR*FCOMN 
      P4M=P4M+RPPOOLR(N,L,NZ,NY,NX)*FCBOXR*FCOMP
      COR=COR+RCPOOLR(N,L,NZ,NY,NX)*FCBCHR*FCHMC 
      Z4M=Z4M+RZPOOLR(N,L,NZ,NY,NX)*FCBCHR*FCHMN 
      P4M=P4M+RPPOOLR(N,L,NZ,NY,NX)*FCBCHR*FCHMP
...
C     CO2G(L,NY,NX)=CO2G(L,NY,NX)+RCMBC*FCBOXR*FCOGC
      ZNH4S(L,NY,NX)=ZNH4S(L,NY,NX)+RCMBN*FCBOXR*FCOMN     
      H2PO4(L,NY,NX)=H2PO4(L,NY,NX)+RCMBP*FCBOXR*FCOMP
C     CH4G(L,NY,NX)=CH4G(L,NY,NX)+RCMBC*FCBCHR*FCHGC
      OSC(5,1,L,NY,NX)=OSC(5,1,L,NY,NX)+RCMBC*FCBCHR*FCHMC
      ZNH4S(L,NY,NX)=ZNH4S(L,NY,NX)+RCMBN*FCBCHR*FCHMN 
      H2PO4(L,NY,NX)=H2PO4(L,NY,NX)+RCMBP*FCBCHR*FCHMP 
      COX=COX+RCMBC*FCBOXR*FCOGC     
      ZOX=ZOX+RCMBN*FCBOXR*FCOGN     
      POX=POX+RCMBP*FCBOXR*FCOGP
      Z4M=Z4M+RCMBN*FCBOXR*FCOMN 
      P4M=P4M+RCMBP*FCBOXR*FCOMP
      CHX=CHX+RCMBC*FCBCHR*FCHGC
      ZOX=ZOX+RCMBN*FCBCHR*FCHGN
      POX=POX+RCMBP*FCBCHR*FCHGP
      COR=COR+RCMBC*FCBCHR*FCHMC 
      Z4M=Z4M+RCMBN*FCBCHR*FCHMN 
      P4M=P4M+RCMBP*FCBCHR*FCHMP
8785  CONTINUE
8885  CONTINUE
...
C    2,COR,COX,CHX,ZOX,Z4M,POX,P4M
C    3,RCGSKR(L,NZ,NY,NX),RCGSK(L,NY,NX),ROGOX(L,NY,NX)
C    4,RCHOX(L,NY,NX),FCBOXR,FCBCHR,FCBOXR+FCBCHR
C    3,VCO2R(NY,NX),VCH4R(NY,NX),VNOXR(NY,NX),VPOXR(NY,NX)
C    4,VCOXF(NZ,NY,NX),VNOXF(NZ,NY,NX),VPOXF(NZ,NY,NX)
C    3,FCBOXR*FCOGC,FCBOXR*FCOGN,FCBOXR*FCOGP,FCBCHR*FCHGC
C    4,FCBCHR*FCHGN,FCBCHR*FCHGP,FCBOXR*FCOMN,FCBOXR*FCOMP
C    5,FCBCHR*FCHMC,FCBCHR*FCHMN,FCBCHR*FCHMP
