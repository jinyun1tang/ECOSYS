...
C
      ...
      PARAMETER (RZS=0.278E-01,RZE=0.139E-01
     2,RACGM=0.556E-02,RACGZ=0.556E-01)
      REAL*4 THETWR,THETW1,THETA1,THETAL,THETWL
     2,TKR1,TKS1,TKY,TKW1,TK0X,TKXR,TK1X,TKX1 
      DO 9995 NX=NHW,NHE
      DO 9990 NY=NVN,NVS
      NUM(NY,NX)=NU(NY,NX)
C
...
C    8,TKGS(1,NY,NX),FSNW(NY,NX),TK0(1,NY,NX),FSNX(NY,NX)
C    9,TKS(0,NY,NX),CVRDW(NY,NX),TKS(NUM(NY,NX),NY,NX),BAREW(NY,NX)
C    1,BARE(NY,NX),XVOLT(NY,NX),VOLWD(NY,NX),VOLW(0,NY,NX),VOLI(0,NY,NX)
C    2,VOLWRX(NY,NX)
...
C        runoff
C     THRMGC,THRMGD=LW radiation emitted to ground surface from
...
C
      ...
      THQS1(NY,NX)=0.0
      FLWRM(M,NY,NX)=0.0
C     DO 900 NZ=1,NP(NY,NX)
C     THRMGC(M,NZ,NY,NX)=0.0
C     THRMGD(M,NZ,NY,NX)=0.0
900   CONTINUE
C
C     CALCULATE CANOPY AIR TEMPERATURE, VAPOR CONCENTRATION
C
C     DTKQ=atmosphere-ground surface air temperature difference
C     DTKG=ground surface air-ground surface temperature difference
...
C
     ...
     2,RIBX(NY,NX)/TKAM(NY,NX)*DTKQ))
      ...
      RACGX=(ZT(NY,NX)*EXP(ALFZ(NY,NX))
     2/(ALFZ(NY,NX)/RAB(NY,NX))*(EXP(-ALFZ(NY,NX)
     3*ZR(NY,NX)/ZT(NY,NX))-EXP(-ALFZ(NY,NX)
     4*(ZD(NY,NX)+ZR(NY,NX))/ZT(NY,NX))))/RIC
      ELSE
      RACGX=0.0
      ENDIF
      RACG(M,NY,NX)=AMIN1(RACGZ,AMAX1(RACGM,RACGX))
C     WRITE(*,101)'RACG',I,J,M,NY,NX,RACG(M,NY,NX),ZT(NY,NX) 
C    2,ALFZ(NY,NX),RAB(NY,NX),ZR(NY,NX),ZT(NY,NX),ZD(NY,NX)
C    3,RABX(NY,NX),RI,RIBX(NY,NX),TKAM(NY,NX),DTKQ,TKQG(M,NY,NX) 
101   FORMAT(A8,5I4,20E12.4) 
     ...
     2,RIBX(NY,NX)/TKQG(M,NY,NX)*DTKG))
      ...
      RZSG=RZS/RIG
C     RZSG=RZS
      FSNW(NY,NX)=AMIN1(1.0,SQRT((AMAX1(0.0,DPTHS0(NY,NX))/DPTHSX)))
      FSNX(NY,NX)=1.0-FSNW(NY,NX)
      BAREW(NY,NX)=AMAX1(0.0,BARE(NY,NX)
     2-AMIN1(1.0,AMAX1(0.0,XVOLT(NY,NX)/VOLWD(NY,NX))))
      CVRDW(NY,NX)=1.0-BAREW(NY,NX)
...
3115  FORMAT(A8,6I4,40E12.4)
      RAGS=1.0/(BAREW(NY,NX)/RZSG+CVRDW(NY,NX)/(RZSG+RZR(NY,NX)))
      PAREWM=PAREW/(RZSG+RZE)
      PARSWM=PARSW/RZSG 
      PAREGM=PAREG/(RAGS+RZE)             
      PARSGM=PARSG/RAGS
      PARERM=PARER/(RZSG+RZE)
      PARSRM=PARSR/RZSG
C     IF(ICHKF.EQ.1)THEN
C     WRITE(*,4423)'RAB',I,J,NFZ,M,NY,NX,PAREGM,PARSGM,PARERM,PARSRM 
C    2,PAREG,PARSG,PARER,PARSR
C    2,RZS,RZE,RZSG,RZR(NY,NX),RAGS,CVRDW(NY,NX)
C    2,RAB(NY,NX),RABM,RABX(NY,NX),RIG,RI
C    2,RIG,RI,RIX,RIY,RIBX(NY,NX),TKGS(M,NY,NX),TKQG(M,NY,NX) 
...
C
     ...
     2,1.0/((Z1R**-1)*EXP(Z2RW*(THETWA-Z3R)))*XNPDX)
      ELSE
      DFGS(M,0,NY,NX)=AMIN1(1.0
     2,1.0/((Z1R**-1)*EXP(Z2RD*(THETWA-Z3R)))*XNPDX)
      ENDIF
      ELSE
      DFGS(M,0,NY,NX)=0.0
      ENDIF
...
C
     ...
     2,1.0/((Z1S**-1)*EXP(Z2SW*(THETWA-Z3S)))*XNPDX) 
      ELSE
      DFGS(M,L,NY,NX)=AMIN1(1.0
     2,1.0/((Z1S**-1)*EXP(Z2SD*(THETWA-Z3S)))*XNPDX) 
      ENDIF
      ELSE
      DFGS(M,L,NY,NX)=0.0
      ENDIF
...
C     
     ...
     2*FRADQ(NZ,NY,NX)
C     THRMGC(M,NZ,NY,NX)=THRMGC(M,NZ,NY,NX)+THRMCW
C     THRMGD(M,NZ,NY,NX)=THRMGD(M,NZ,NY,NX)+THRMDW
...
C    5,TKC(NZ,NY,NX),TKD(NZ,NY,NX),TK0M(1,NY,NX)
...
C
     ...
     2*0.61*EXP(5360.0*(3.661E-03-1.0/TK0M(1,NY,NX)))
      EVAPT2=PAREWM*(VPQG(M,NY,NX)-VP0)
      ...
      ELSE
      VFLXW2=(EVAPW2*4.19+EVAPS2*2.095)*TKQG(M,NY,NX)
      ENDIF
C
...
C
      SFLXW2=PARSWM*(TKQG(M,NY,NX)-TK0M(1,NY,NX))
...
C    3,FSNW(NY,NX),DPTHS0(NY,NX),DPTHSX,(DLYRS0(L,NY,NX),L=1,5)
C    4,FLW0I2,FLQ0I2
C    4,RFLXW2,EFLXW2,SFLXW2,VFLXW2,EVAPT2,EVAPX2,VPQG(M,NY,NX) 
C    5,VP0,VOLW0M(1,NY,NX),VOLS0M(1,NY,NX),VOLI0M(1,NY,NX)
C    6,HFLW0W(1,NY,NX),HFLW0W2,HWFLQ02,HFLXW2,RFLXW2,EFLXW2
C    7,TK0M(1,NY,NX),TKQG(M,NY,NX),VHCPWMM(1,NY,NX)
C    8,RZS,EVAPS2,EVAPW2,EVAPT2,PARSWM,PAREWM,RZSG,RZS,RIG
C    9,TKGS(M,NY,NX),TK0(1,NY,NX),TKQG(M,NY,NX) 
7759  FORMAT(A8,7I4,60E14.6) 
C     ENDIF 
C
...
C    3,HFLC0R,HFLCR1,VPQG(M,NY,NX),VP0,VPR,VP1,PSISM1(0,NY,NX),PSISV1
...
C     IF(IYRC.EQ.1983.AND.L.EQ.1)THEN
...
C
     ...
     3+1.9274*VOLI0M(L,NY,NX)
      ...
      ELSEIF(L.EQ.1)THEN
      TK0M(L,NY,NX)=TKQG(M,NY,NX)
      ELSE
      TK0M(L,NY,NX)=TK0M(L-1,NY,NX)
      ENDIF
C     IF(IYRC.EQ.1983.AND.I.GT.300.AND.L.EQ.1)THEN
C     WRITE(*,7758)'TK0M',I,J,NFZ,M,MM,NX,NY,L,TK0M(L,NY,NX)
...
C    2,THFLWWX,HFLF0X,HFLVX,HFLW0W(L,NY,NX),HFLW0W(L2,NY,NX) 
...
C     BKDS=bulk density (0=pond)
C     VHCP1,VHCPRX=current,minimum surface layer heat capacities
C
...
C
     ...
     2*FRADQ(NZ,NY,NX)
C     THRMGC(M,NZ,NY,NX)=THRMGC(M,NZ,NY,NX)+THRMCS
C     THRMGD(M,NZ,NY,NX)=THRMGD(M,NZ,NY,NX)+THRMDS
...
C
     ...
     3*EXP(18.0*PSISV1/(8.3143*TKX1))
      EVAPG(NY,NX)=AMAX1(PAREGM*(VPQG(M,NY,NX)-VP1)
...
C
      SFLXG=PARSGM*(TKQG(M,NY,NX)-TK1(NUM(NY,NX),NY,NX))
      HFLX0=RFLXG+EFLXG+SFLXG
      HFLXG=HFLX0+VFLXG
C     IF(NX.EQ.5)THEN
C     WRITE(*,1112)'EFLXG',I,J,NFZ,M,NX,NY,NUM(NY,NX) 
C    2,EVAPG(NY,NX),TK1(0,NY,NX),TK1(NUM(NY,NX),NY,NX),TKQG(M,NY,NX) 
C    2,DLYRR(NY,NX),RFLXR,RFLXG,EFLXG,SFLXG,VFLXG,HFLX0,HFLXG 
C    3,RZSG,RAGS,RZE,RI,RIG,RZR(NY,NX),PAREGM,PARSGM
...
C    5,FRADG(NY,NX),VPQG(M,NY,NX),VP1,PSISV1,FLQM 
C    6,PARSGM,PAREGM,HWFLQM,BAREW(NY,NX),RZSG,CVRDW(NY,NX),RZR(NY,NX)
...
C     VHCP12,VHCPRX=current,minimum surface layer heat capacities
C     TKR1,TKS1=litter,soil surface interim temperature
C     HFLXF=heat released by combustion in previous time step (MJ m-2)
C
      DO 5000 NN=1,NPR
      IF(VHCPR2.GT.VHCPRX(NY,NX).AND.VHCP12.GT.VHCPRX(NY,NX))THEN
...
C
     ...
     2*FRADQ(NZ,NY,NX)
C     THRMGC(M,NZ,NY,NX)=THRMGC(M,NZ,NY,NX)+THRMCR2
C     THRMGD(M,NZ,NY,NX)=THRMGD(M,NZ,NY,NX)+THRMDR2
...
C
      SFLXR2=PARSRM*(TKQG(M,NY,NX)-TKR1)
...
C
     ...
     2+1.9274*VOLI1(0,NY,NX)
      ...
      IF(VHCP12.GT.VHCPRX(NY,NX))THEN
...
C    2,TKR1,TKS1,TKQG(M,NY,NX),RFLXR2,EFLXR2,SFLXR2,VFLXR2
...
C    2,VPQG(M,NY,NX),VPR1,EVAPR(NY,NX),EVAPR2,VOLWR2,XNPXX 
...
C    4,VHCPRX(NY,NX),PARSRM,PARERM,HFLXF(0,NY,NX)
C    5,RA,RZS,RI,TKQG(M,NY,NX),VOLW1(0,NY,NX) 
...
C
      ...
      DTKQC=TKQC(NZ,NY,NX)-TKQG(M,NY,NX)
      DVPQC=VPQC(NZ,NY,NX)-VPQG(M,NY,NX)
      RI=AMAX1(RIX,AMIN1(RIY
     2,RIBX(NY,NX)/TKQC(NZ,NY,NX)*DTKQC))
      RIC=1.0-3.3*RI 
      DTKQD=TKQD(NZ,NY,NX)-TKQG(M,NY,NX) 
      DVPQD=VPQD(NZ,NY,NX)-VPQG(M,NY,NX) 
      RI=AMAX1(RIX,AMIN1(RIY
     2,RIBX(NY,NX)/TKQD(NZ,NY,NX)*DTKQD))
      RID=1.0-3.3*RI 
      IF(ZC(NZ,NY,NX).EQ.ZT(NY,NX))THEN
      RAGCX=RACGX
      ELSEIF(ZC(NZ,NY,NX).GT.ZERO)THEN
      RAGCX=(ZC(NZ,NY,NX)*EXP(ALFZ(NY,NX))
     2/(ALFZ(NY,NX)/RAB(NY,NX))*(EXP(-ALFZ(NY,NX)
     3*ZR(NY,NX)/ZT(NY,NX))-EXP(-ALFZ(NY,NX)
     4*(ZD(NY,NX)+ZR(NY,NX))/ZC(NZ,NY,NX))))/RIC 
      ELSE
      RAGCX=0.0
      ENDIF
      RAGC(M,NZ,NY,NX)=AMIN1(RACGZ,AMAX1(RACGM,RAGCX))
      IF(ZG(NZ,NY,NX).EQ.ZT(NY,NX))THEN
      RAGDX=RACGX
      ELSEIF(ZG(NZ,NY,NX).GT.ZERO)THEN
      RAGDX=(ZG(NZ,NY,NX)*EXP(ALFZ(NY,NX))
     2/(ALFZ(NY,NX)/RAB(NY,NX))*(EXP(-ALFZ(NY,NX)
     3*ZR(NY,NX)/ZT(NY,NX))-EXP(-ALFZ(NY,NX)
     4*(ZD(NY,NX)+ZR(NY,NX))/ZG(NZ,NY,NX))))/RID 
      ELSE
      RAGDX=0.0
      ENDIF
      RAGD(M,NZ,NY,NX)=AMIN1(RACGZ,AMAX1(RACGM,RAGDX))
920   CONTINUE
      IF(M.LT.NPH)THEN
      SHGCM=0.0
      SHGDM=0.0
      ...
      SHGCM=SHGCM+PARSCM*DTKQC*FLAIP(NZ,NY,NX)
      VPGCM=VPGCM+PARECM*DVPQC*FLAIP(NZ,NY,NX)
      SHGDM=SHGDM+PARSDM*DTKQD*FLAIQ(NZ,NY,NX)
      VPGDM=VPGDM+PAREDM*DVPQD*FLAIQ(NZ,NY,NX)
921   CONTINUE
      SFLXA=DTKQ*AMIN1(PARSX(NY,NX)/RATG,VHCPQ(NY,NX))
      TSHGM=SFLXA+SHGCM+SHGDM-SFLXG-SFLXR-SFLXW 
      DVPQ=VPAM(NY,NX)-VPQG(M,NY,NX)
      EVAPA=DVPQ*AMIN1(PAREX(NY,NX)/RATG,EVAPQ(NY,NX))
      TEVGM=EVAPA+VPGCM+VPGDM 
     2-EVAPG(NY,NX)-EVAPR(NY,NX)-EVAPW(NY,NX)-EVAPS(NY,NX)
...
C    2,TKQG(M+1,NY,NX),TKAM(NY,NX),TKGS(M,NY,NX),VPQG(M+1,NY,NX) 
C    3,VPSG,VPAM(NY,NX),TK1(0,NY,NX),TK1(NUM(NY,NX),NY,NX) 
C    4,BAREW(NY,NX),VHCP1(0,NY,NX),VHCP1(NUM(NY,NX),NY,NX) 
C    3,VHCPRX(NY,NX),AREA(3,NU(NY,NX),NY,NX)
C    5,VOLW10,VOLI10,FLWRL(NY,NX),WFLVR(NY,NX),WFLFR(NY,NX) 
C    3,FRADT(NY,NX),VHCPQ(NY,NX),EVAPQ(NY,NX)
C    2,RAB(NY,NX),RACGX,RACG(M,NY,NX),RZS,RZSG,RZR(NY,NX)
C    3,TEVGM,EVAPA,VPGCM,VPGDM,PARECM,DVPQC,PAREDM,DVPQD
C    4,TSHGM,SHGCM,SHGDM,SFLXR,SFLXG,SFLXW,SFLXA,SHGCM,SHGDM 
...
C    4,SFLXA,DTKQ,PARSX(NY,NX)/RATG,VHCPQ(NY,NX)
C    5,HEATH(NY,NX),RFLXG,RFLXR,RFLXW
C    2,SFLXG,SFLXR,SFLXW,EFLXG,EFLXR,EFLXW,VFLXG,VFLXR,VFLXW
C    4,XVOLT(NY,NX),FLWR(NY,NX),ORGC(0,NY,NX)
C    3,ORGCC(0,NY,NX),VOLW(0,NY,NX),VOLV(0,NY,NX),VOLI(0,NY,NX)
...
C
      PARR(M,NY,NX)=PAREX(NY,NX)/(RATG+RZSG+RAS(NY,NX))
...
C     IF(J.EQ.15.AND.N3.EQ.NU(N2,N1))THEN
C     WRITE(*,1115)'FLWL',I,J,NFZ,M,N1,N2,N3,N4,N5,N6,N,K1,KL
C    2,FLWL(N,N3,N2,N1),FLWL(N,N6,N5,N4),FLQX,FLQZ,FLQL,FLQ2 
C    3,PSIST1,PSISTL,XNPHX,VOLP1Z(N6,N5,N4)
C    3,FLVC,VP1,VPL,VPY,FLVX,FLVQ
C    4,AVCNDL,CND1,CNDL,FKSAT
C    2,VOLP1(N3,N2,N1),VOLP1(N6,N5,N4),VOLW2(N3,N2,N1)
C    3,VOLWX1(N3,N2,N1),VOLW2(N6,N5,N4),VOLWX1(N6,N5,N4)
...
C    2,PSISM1(L,NY,NX),0.0098*(DPTH(L,NY,NX)-DTBLX(NY,NX)),DTBLXX 
...
C
     ...
     3.OR.(CDPTH(NU(N2,N1)-1,N2,N1).GT.CDPTHI(N2,N1)
...
C
     ...
     3*EXP(18.0*PSISV1/(8.3143*TK1(N3,N2,N1)))
      ...
      ENDIF 
C     IF(N3.EQ.14)THEN     
C     WRITE(*,7762)'WFLVL',I,J,NFZ,M,N1,N2,N3,VOLW1X,VOLV1X
C    2,FLVLX,FLVLW,HFLVLW,TK1(N3,N2,N1),VPL
C    2,PSISM1(N3,N2,N1),VOLP1(N3,N2,N1),VPL*VOLP1(N3,N2,N1)
C    3,VOLV1(N3,N2,N1),TFLVL(N3,N2,N1),VHCP1(N3,N2,N1)
...
C
     ...
     2/((1.0+6.2913E-03*TFREEZ)*(1.0-0.00*PSISMX))*XNPXX
...
C    4,VOLW1X,VOLI1X,VOLW1(N3,N2,N1),VOLI1(N3,N2,N1)
...
C
     ...
     3/LOG(PHOL(N3,N2,N1)/HRAD(N3,N2,N1))*XNPHX 
      VOLW1X=VOLW1(N3,N2,N1)+TFLWL(N3,N2,N1)+TWFLVL(N3,N2,N1) 
     2+TWFLFL(N3,N2,N1)+FLU1(N3,N2,N1)
      VOLP1X=AMAX1(0.0,VOLA1(N3,N2,N1)-VOLW1X-VOLI1(N3,N2,N1))
      VOLWH1X=VOLWH1(N3,N2,N1)+TFLWHL(N3,N2,N1)+TWFLFH(N3,N2,N1) 
...
C    3,FINHX,VOLWH1X,VOLP1X,VOLPH1X,VOLW1X
...
C    6,VOLA1(N3,N2,N1),VOLW1X,VOLI1(N3,N2,N1)
...
C    3,TK1(0,NY,NX),TKAM(NY,NX),HFLWRL(NY,NX)
C    3,HFLVR(NY,NX),HFLVR(NY,NX),THQR1(NY,NX),HFLXF(0,NY,NX)
C    4,VHCP1(0,NY,NX) 
C    3,ENGYR,VHCP1(0,NY,NX),VHCPRX(NY,NX),CVRD(NY,NX)
C    2,HFLWRLG,HFLWRLW,HWFLYM,HFLXR,HFLWR(NY,NX)
C    3,BARE(NY,NX),ORGC(0,NY,NX),ORGCC(0,NY,NX)
C    4,VHCP1(NUM(NY,NX),NY,NX),TK1(NUM(NY,NX),NY,NX) 
...
C    3,TWFLFL(L,NY,NX),TWFLVL(L,NY,NX),WFLVL(L,NY,NX)
