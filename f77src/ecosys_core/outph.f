...
C
      ...
      M=M+1
      IF(K.EQ.51)HEAD(M)=HCNET(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)*23.148
...
C
      ...
      IF(K.EQ.55)HEAD(M)=TTRAN(NZ,NY,NX)*1000.0
...
C
      ...
      IF(CHOICE(K,N-20).EQ.'YES')THEN
      M=M+1
      IF(K.EQ.51)HEAD(M)=277.8*TRNP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.52)HEAD(M)=277.8*TLEP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.53)HEAD(M)=277.8*TSHP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.54)HEAD(M)=277.8*TGHP(NZ,NY,NX)/AREA(3,NU(NY,NX),NY,NX)
      IF(K.EQ.55)HEAD(M)=TCC(NZ,NY,NX)
      IF(K.EQ.56)HEAD(M)=TFN3(NZ,NY,NX)
      IF(K.EQ.57)HEAD(M)=TCD(NZ,NY,NX)
      ENDIF
1025  CONTINUE
      WRITE(LUN,'(A16,F8.3,4X,A8,I8,50E16.7E3)')OUTFILP(N-20,NZ,NY,NX)
     2,DOY,CDATE,J,(HEAD(K),K=1,M)
