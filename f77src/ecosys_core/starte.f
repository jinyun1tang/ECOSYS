...
C
      ...
      CCLZ=CCLQ(I,NY,NX)
C
C     INITIALIZE IONS IN SOIL WATER
C
...
C     A*=ion activity coefficients
C     ISALTG=salt option flag from site file
C
...
C     FROM GAPON SELECTIVITY CO-EFFICIENTS
C
C     GKC4,GKCH,GKCA,GKCM,GKCN,GKCK=Gapon selectivity coefficients for
C     Ca-NH4,Ca-H,Ca-Al,Ca-Mg,Ca-Na,Ca-K from soil file
C     VLPO4,VLPOB=fraction of soil volume in non-band,band zones
C     XCEC=cation exchange capacity from soil file
C
      IF(K.EQ.3)THEN
      PALOH1=CALOHX
      PFEOH1=CFEOHX
...
C
      ...
      CH2P1=AMAX1(ZERO,CH2P1)
C
C     PRECIPITATION-DISSOLUTION FLUXES FROM SOLUBILITY PRODUCTS
C
      IF(K.EQ.3)THEN
      ...
      PCAPH1=PCAPH1+RPCAHX
C
C     ANION EXCHANGE FLUXES FROM SOLUBILITY PRODUCTS
C
      IF(VOLW(L,NY,NX).GT.ZEROS(NY,NX))THEN
      VOLWBK=AMIN1(1.0,BKVLX/VOLW(L,NY,NX))
      ELSE
     ...
     2/(XOH11+SXH2P*COH1)*VOLWBK 
      ...
      ENDIF
C
C     CATION EXCHANGE FROM SOLUBILITY PRODUCTS
C
      IF(XCEC(L,NY,NX).GT.ZEROS(NY,NX))THEN
      CALX=CAL1**0.333
      CFEX=CFE1**0.333
     ...
     5+GKCN(L,NY,NX)*CNA1/CCAX+GKCK(L,NY,NX)*CKA1/CCAX)
      ...
      ENDIF
C
C     SOLUTE ION EQUILIBRIA FROM DISSOCIATION CONSTANTS
C
C     NH4 – NH3 
C
      S0=CHY1+CN31+DPN4
...
C
      ...
      RH2P=TSL*(S0-SQRT(S1))
C
C     TOTAL ION FLUXES FROM ALL PRECIPITATION-DISSOLUTION, EXCHANGE
C     AND SOLUTE EQUILIBRIUM REACTIONS
C
      RN4S=RNH4-RXN4
      RN3S=-RNH4
      RHP1=-RXH1P-RH2P 
...
C
C     XN4,XHY,XAL,XFE,XCA,XMG,XNA,XKA,XHC,XALO2,XFEO2,XOH0,XOH1,XOH2
C        XH1P,XH2P=exchangeable NH4,H,AL,FE,CA,MG,NA,K,HCO3,AL(OH)2
C        FE(OH)2,O,OH,OH2,HPO4,H2PO4
C     VLNH4,VLNHB=fractions of soil volume in NH4 non-band,band
C     VLNO3,VLNOB=fractions of soil volume in NO3 non-band,band
C     VLPO4,VLPOB=fractions of soil volume in H2PO4 non-band,band
C
...
C
C     INITIAL STATE VARIABLES FOR PRECIPITATES P*
C
...
C
      IF(DATA(20).EQ.'NO'.AND.IGO.EQ.0)THEN
C
C     AQUEOUS (*S) AND GASEOUS (*G) GASES
C
