...
195   CONTINUE
      ...
      RDXMP=0.0
      DO 1191 K=0,4
      RDXMC=RDXMC+RDFOMC(N,K,L,NZ,NY,NX)
      RDXMN=RDXMN+RDFOMN(N,K,L,NZ,NY,NX)
      RDXMP=RDXMP+RDFOMP(N,K,L,NZ,NY,NX)
1191  CONTINUE
      IF(RDXMC<0.0)THEN
      IF(RDXMC+CPOOLR(N,L,NZ,NY,NX)<0.0)THEN
      SCALF=CPOOLR(N,L,NZ,NY,NX)/(-RDXMC)*0.999999
      DO K=0,4
      RDFOMC(N,K,L,NZ,NY,NX)=RDFOMC(N,K,L,NZ,NY,NX)*SCALF
      ENDDO
      ENDIF
      ENDIF
      IF(RDXMN<0.0)THEN
      IF(RDXMN+ZPOOLR(N,L,NZ,NY,NX)<0.0)THEN
      SCALF=ZPOOLR(N,L,NZ,NY,NX)/(-RDXMN)*0.999999
      DO K=0,4
      RDFOMN(N,K,L,NZ,NY,NX)=RDFOMN(N,K,L,NZ,NY,NX)*SCALF
      ENDDO
      ENDIF
      ENDIF
      IF(RDXMP<0.0)THEN
      IF(RDXMP+PPOOLR(N,L,NZ,NY,NX)<0.0)THEN
      SCALF=PPOOLR(N,L,NZ,NY,NX)/(-RDXMP)*0.999999
      DO K=0,4
      RDFOMP(N,K,L,NZ,NY,NX)=RDFOMP(N,K,L,NZ,NY,NX)*SCALF
      ENDDO
      ENDIF
      ENDIF
