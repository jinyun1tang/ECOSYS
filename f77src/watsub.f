...
C
     ...
     9,VOLIH1(JZ,JY,JX),THETPY(0:JZ,JY,JX)  
       PARAMETER (THETPI=0.00,EMMS=0.98,EMMW=0.98,EMMR=0.98
...
C    4,(Z1S**-1)*EXP(Z2S*(THETWA-Z3S))
      ...
      TORT(M,L,NY,NX)=XDIM*0.7*THETWT**2*(1.0-FHOL(L,NY,NX))
      IF(VOLAH(L,NY,NX).GT.ZEROS(NY,NX))THEN
...
C    2,VOLI1(0,NY,NX),VOLP1(0,NY,NX),FLWRL(NY,NX),WFLXR(NY,NX)
C    2,TQR1(NY,NX),TRC0(NY,NX),VHCPR1(NY,NX),VHCPRX(NY,NX),CVRD(NY,NX)
C    4,FLWR(NY,NX),VOLA(0,NY,NX),VOLWRX(NY,NX),VOLR(NY,NX)
C    2,ORGC(0,NY,NX),PSISM1(0,NY,NX)
...
C    2+THETPI*(VOLI1(L,NY,NX)+VOLIH1(L,NY,NX))
