...
C
     ...
     4,VHCP1(0:JZ,JY,JX),TK1(0:JZ,JY,JX),TWFLXL(JZ,JY,JX) 
     ...
     2,RAGR(JY,JX),RAGW(JY,JX),PAREG(JY,JX) 
     ...
     7,TQI1(JY,JX),THQS1(JY,JX),EVAP(JY,JX),TTFLXL(JZ,JY,JX)
     ...
     9,VOLIH1(JZ,JY,JX),THETPY(0:JZ,JY,JX),FLWNX(JY,JX)
     1,FLWXNX(JY,JX),FLWHNX(JY,JX),HFLWNX(JY,JX),N6X(JY,JX) 
     ...
     7,TRBW=0.375,TRBA=0.000)
      PARAMETER (FHFLX=1.0,FVOLAH=0.0,PSISX=-0.5,PSISXR=-0.5
...
C    2,TRC0(NY,NX)/AREA(3,0,NY,NX),TVOLG(NY,NX),VOLWG(NY,NX)
...
C
     ...
     2-0.05*AMAX1(0.0,VOLSS(NY,NX)))
      ...
      FLWQGS=FLWQG*FGRD(NUM(NY,NX),NY,NX)
      FLWQGH=FLWQG*FMAC(NUM(NY,NX),NY,NX)
      ELSE
      WMELT=0.0
      FLWQR=0.0
      HFLWQR=0.0
...
C
      ...
      THRMW(NY,NX)=EMMW*2.04E-10*AREA(3,NUM(NY,NX),NY,NX)*XNPH
      THRMS(NY,NX)=EMMS*2.04E-10*AREA(3,NUM(NY,NX),NY,NX)*XNPH
     2*BARE(NY,NX)
      THRMR(NY,NX)=EMMR*2.04E-10*AREA(3,NUM(NY,NX),NY,NX)*XNPHR
     2*CVRD(NY,NX)
C
...
C     IF(VOLR(NY,NX).GT.ZEROS(NY,NX)
C    2.AND.VHCP1(0,NY,NX).GT.VHCPRX(NY,NX))THEN
      RAR1=RAR(NY,NX)/AMAX1(THETX,THETPX(0,NY,NX))**2.33*CVRD(NY,NX)
C     ELSE
C     RAR1=0.0
C     ENDIF
      RAGS(NY,NX)=RAG(NY,NX)+RAR1
      PARR(NY,NX)=AREA(3,NUM(NY,NX),NY,NX)*XNPH/RAGR(NY,NX)
      PARG(NY,NX)=AREA(3,NUM(NY,NX),NY,NX)*XNPH/RAGS(NY,NX)
      PAREG(NY,NX)=AREA(3,NUM(NY,NX),NY,NX)*XNPH
      PARER(NY,NX)=PAREG(NY,NX)*XNPR*CVRD(NY,NX)
      PARSG(NY,NX)=1.25E-03*AREA(3,NUM(NY,NX),NY,NX)*XNPH
...
C    6,TVOLW(NY,NX),VHCPRX(NY,NX)/4.19,VOLWD(NY,NX),TRC0(NY,NX)
3111  FORMAT(A8,2I4,40E12.4)
C     ENDIF
9990  CONTINUE
9995  CONTINUE
C
...
C    4,(Z1S**-1)*EXP(Z2S*(THETWA-Z3S))
      IF(BKDS(L,NY,NX).GT.ZEROS(NY,NX))THEN
...
C
      ...
      FLQM=FLQ1(NY,NX)+FLWR1*FGRD(NUM(NY,NX),NY,NX)
      FLHM=FLH1(NY,NX)+FLWR1*FMAC(NUM(NY,NX),NY,NX)
...
C
     ...
     2/AREA(3,NUM(NY,NX),NY,NX))
      ...
      DPTHS0(NY,NX)=VOLS1(NY,NX)/AREA(3,NUM(NY,NX),NY,NX)
     ...
     2,VOLW1(NUM(NY,NX),NY,NX)/VOLXI(NUM(NY,NX),NY,NX)))
      IF(BKVL(NUM(NY,NX),NY,NX).GT.0.0)THEN
...
C
      ALBW=(0.80*VOLS0(NY,NX)+0.30*VOLI0(NY,NX)+0.06*VOLW0(NY,NX))
...
C
      ...
      CNV1=THETPX(NUM(NY,NX),NY,NX)**2/POROQ(NUM(NY,NX),NY,NX)
     2*WGSG1(NUM(NY,NX),NY,NX)
      IF(CNV0.GT.ZERO.AND.CNV1.GT.ZERO)THEN
      AVCNV1=2.0*CNV0*CNV1
     2/(CNV0*DLYR(3,NUM(NY,NX),NY,NX)+CNV1*DPTHS0(NY,NX))
      ELSE
      AVCNV1=2.0*CNV0
     2/(DLYR(3,NUM(NY,NX),NY,NX)+DPTHS0(NY,NX))
      ENDIF
C
C     HEAT AND VAPOR FLUXES BETWEEN SNOWPACK AND SOIL SURFACE
C
      TKY=(TK0(NY,NX)*VHCP0(NY,NX)+TK1(NUM(NY,NX),NY,NX)
     2*VHCP1(NUM(NY,NX),NY,NX))/(VHCP0(NY,NX)+VHCP1(NUM(NY,NX),NY,NX))
      HFLWX=(TK0(NY,NX)-TKY)*VHCP0(NY,NX)*FHFLX*XDIM 
      FLVX=AVCNV1*(VP0-VP1)*AREA(3,NUM(NY,NX),NY,NX)*BARE(NY,NX) 
      IF(FLVX.GE.0.0)THEN
      FLV1=AMIN1(FLVX,VOLW0(NY,NX)*XH(3,0,NY,NX))
      IF(HFLWX.GE.0.0)THEN
      FLV1=AMIN1(FLV1,HFLWX/(4.19*TK0(NY,NX)+VAP))
      ENDIF
      HWFLV1=(4.19*TK0(NY,NX)+VAP)*FLV1
      ELSE
      FLV1=AMAX1(FLVX,-VOLW1(NUM(NY,NX),NY,NX)*XH(3,NUM(NY,NX),NY,NX))
      IF(HFLWX.LT.0.0)THEN
      FLV1=AMAX1(FLV1,HFLWX/(4.19*TK1(NUM(NY,NX),NY,NX)+VAP))
      ENDIF
      HWFLV1=(4.19*TK1(NUM(NY,NX),NY,NX)+VAP)*FLV1
      ENDIF
...
C
      ...
      TCND1=(STC(NUM(NY,NX),NY,NX)+THETWX(NUM(NY,NX),NY,NX)*2.067E-03
     2+0.611*THETIX(NUM(NY,NX),NY,NX)*7.844E-03
     3+WTHET1*THETPX(NUM(NY,NX),NY,NX)*9.050E-05)
     ...
     5+0.611*THETIX(NUM(NY,NX),NY,NX)+WTHET1*THETPX(NUM(NY,NX),NY,NX))
      ...
      TCNDW1=TCNDW*XNPH
      TCND1W=TCND1*XNPH
      ATCND0=2.0*TCNDW1*TCND1W/(TCNDW1*DLYR(3,NUM(NY,NX),NY,NX)
     2+TCND1W*DPTHS0(NY,NX))*BARE(NY,NX) 
      ELSE
      ATCND0=0.0
      ENDIF
      TK0X=TK0(NY,NX)-HWFLV1/VHCP0(NY,NX)
      TK1X=TK1(NUM(NY,NX),NY,NX)+HWFLV1/VHCP1(NUM(NY,NX),NY,NX) 
      TKY=(TK0X*VHCP0(NY,NX)+TK1X*VHCP1(NUM(NY,NX),NY,NX))
     2/(VHCP0(NY,NX)+VHCP1(NUM(NY,NX),NY,NX))
      HFLWX=(TK0X-TKY)*VHCP0(NY,NX)*FHFLX*XDIM 
      HFLWC=ATCND0*(TK0X-TK1X)*AREA(3,NUM(NY,NX),NY,NX)
...
C    6,WGSG0(NY,NX),WGSG1(NUM(NY,NX),NY,NX),DPTHS0(NY,NX)
C    7,VOLW1(NUM(NY,NX),NY,NX),FLQM,FLYM,WMELT
C    8,HWFLQM,HWFLV1,HFLC01,HFLCR1 
C    9,WGSG0(NY,NX),THETPY(NUM(NY,NX),NY,NX)
...
C
     ...
     2/(CNVR1*DLYR(3,NUM(NY,NX),NY,NX)+CNV11*DLYRR(NY,NX))*CVRD(NY,NX)
      ELSE
      AVCNVS=2.0*CNV11
     ...
     2/(TCNDR1*DLYR(3,NUM(NY,NX),NY,NX)+TCND11*DLYRR(NY,NX))*CVRD(NY,NX) 
      ELSE
      ATCNDS=0.0
      ENDIF
      ELSE
     ...
     3*EXP(18.0*PSISM1(0,NY,NX)/(8.3143*TKXR))
      TKY=(TKXR*VHCP1(0,NY,NX)+TK0X*VHCP0(NY,NX))
     2/(VHCP1(0,NY,NX)+VHCP0(NY,NX)) 
      HFLWX=(TKY-TKXR)*VHCP1(0,NY,NX)*FHFLX*XDIM 
      FLVX=AVCNVR*(VP0-VPR)*AREA(3,NUM(NY,NX),NY,NX) 
      IF(FLVX.GE.0.0)THEN
      FLVR1=AMIN1(FLVX,VOLW0(NY,NX)*XHS(NY,NX))
      IF(HFLWX.GE.0.0)THEN
      FLVR1=AMIN1(FLVR1,HFLWX/(4.19*TK0X+VAP))
      ENDIF
      HWFLVR1=(4.19*TK0X+VAP)*FLVR1
      ELSE
      FLVR1=AMAX1(FLVX,-VOLW1(0,NY,NX)*XH(3,0,NY,NX))
      IF(HFLWX.LT.0.0)THEN
      ...
      TKXR=TKXR+HWFLVR1/VHCP1(0,NY,NX) 
      TKY=(TKXR*VHCP1(0,NY,NX)+TK0X*VHCP0(NY,NX))
     2/(VHCP1(0,NY,NX)+VHCP0(NY,NX)) 
      HFLWX=(TKY-TKXR)*VHCP1(0,NY,NX)*FHFLX*XDIM 
      ...
      TKXR=TKXR+HFLC0R1/VHCP1(0,NY,NX) 
C
...
C
     ...
     2/(VHCP1(0,NY,NX)+VHCP1(NUM(NY,NX),NY,NX))
      HFLWX=(TKXR-TKY)*VHCP1(0,NY,NX)*FHFLX*XDIM 
      FLVX=AVCNVS*(VPR-VP1)*AREA(3,NUM(NY,NX),NY,NX) 
      IF(FLVX.GE.0.0)THEN
      FLVS1=AMIN1(FLVX,VOLW1(0,NY,NX)*XH(3,0,NY,NX))
      IF(HFLWX.GE.0.0)THEN
      FLVS1=AMIN1(FLVS1,HFLWX/(4.19*TKXR+VAP))
      ENDIF
      HWFLVS1=(4.19*TKXR+VAP)*FLVS1
      ELSE
      FLVS1=AMAX1(FLVX,-VOLW1(NUM(NY,NX),NY,NX)*XH(3,NUM(NY,NX),NY,NX))
      IF(HFLWX.LT.0.0)THEN
      FLVS1=AMAX1(FLVS1,HFLWX/(4.19*TK1X+VAP))
      ENDIF
      HWFLVS1=(4.19*TK1X+VAP)*FLVS1
      ENDIF
      TKXR=TKXR-HWFLVS1/VHCP1(0,NY,NX)
      TK1X=TK1X+HWFLVS1/VHCP1(NUM(NY,NX),NY,NX) 
      TKY=(TKXR*VHCP1(0,NY,NX)+TK1X*VHCP1(NUM(NY,NX),NY,NX))
     2/(VHCP1(0,NY,NX)+VHCP1(NUM(NY,NX),NY,NX))
      HFLWX=(TKXR-TKY)*VHCP1(0,NY,NX)*FHFLX*XDIM 
      ...
      ENDIF
      TKXR=TKXR-HFLCR11/VHCP1(0,NY,NX)
      TK1X=TK1X+HFLCR11/VHCP1(NUM(NY,NX),NY,NX) 
      FLVR=FLVR+FLVR1
...
C    5,AVCNVR,ATCNDR,AVCNVS,ATCNDS,VHCP0(NY,NX),VHCP1(0,NY,NX)
C    6,VHCP1(NUM(NY,NX),NY,NX),DLYRR(NY,NX),DPTHS0(NY,NX),CNV01,CNVR1
C    7,CNV11,CNV1,THETPX(NUM(NY,NX),NY,NX),POROQ(NUM(NY,NX),NY,NX)
C    2,WGSG1(NUM(NY,NX),NY,NX),CVRD(NY,NX),HFLR1,HWFLV1,HFLCR1
...
4000  CONTINUE
      ...
      ELSE
      PSISM1(0,NY,NX)=PSISM1(NUM(NY,NX),NY,NX)
      ENDIF
      EVAPR(NY,NX)=0.0
      RFLXR=0.0
      EFLXR=0.0
...
C
      ...
      FLWL(3,NUM(NY,NX),NY,NX)=FLQM+FLV1+FLVS
      FLWLX(3,NUM(NY,NX),NY,NX)=FLQM+FLV1
      FLWHL(3,NUM(NY,NX),NY,NX)=FLHM
      HFLWL(3,NUM(NY,NX),NY,NX)=HWFLQM+HWFLV1+HWFLVS+HFLC01+HFLCR1
      FLWRL(NY,NX)=FLYM+FLVR-FLVS
      HFLWRL(NY,NX)=HWFLYM+HFLC0R-HFLCR1+HWFLVR-HWFLVS
      FLWVL(NUM(NY,NX),NY,NX)=0.0
      FLWV(NUM(NY,NX),NY,NX)=FLWV(NUM(NY,NX),NY,NX)
     2+FLWVL(NUM(NY,NX),NY,NX)
C     IF(I.EQ.328)THEN
C     WRITE(*,7753)'FLWL',I,J,M,NX,NY,NUM(NY,NX)
C    2,FLWL(3,NUM(NY,NX),NY,NX),FLQM,FLV1,FLVS 
C     WRITE(*,7753)'FLW0L',I,J,M,NX,NY,NUM(NY,NX),FLW0L(NY,NX)
...
C
     ...
     2+VHCP0(NY,NX)*(273.15-TK0(NY,NX))-HFLW0L(NY,NX))
      ...
      FVOLI0=0.0
      ENDIF
      TFLX0(NY,NX)=AMAX1(-333.0*TVOLWS*XHS(NY,NX),TFLX)
      WFLXA(NY,NX)=-TFLX0(NY,NX)*FVOLS0/333.0
      WFLXB(NY,NX)=-TFLX0(NY,NX)*FVOLI0/333.0
      ELSE
      TFLX0(NY,NX)=AMIN1(333.0*VOLW0(NY,NX)*XHS(NY,NX),TFLX)
      WFLXA(NY,NX)=0.0
...
C
     ...
     2,-AMAX1(0.0,VOLW1(NUM(NY,NX),NY,NX)*XH(3,NUM(NY,NX),NY,NX)))
      ...
      VFLX=EVAP(NY,NX)*4.19*TK1(NUM(NY,NX),NY,NX)
      ELSE
...
C     BALANCE OCCURS, SOLVE LATENT, SENSIBLE AND STORAGE HEAT FLUXES
C
      SFLX=PARS*(TKQ(NY,NX)-TK1(NUM(NY,NX),NY,NX))
      HFLX1=RFLX+EFLX+SFLX
      HFLW1=HFLX1+VFLX
C     IF(J.EQ.12.AND.NX.EQ.2.AND.M.EQ.NPH)THEN
C     WRITE(*,1112)'EFLX',I,J,M,NX,NY,NUM(NY,NX),TK1(NUM(NY,NX),NY,NX) 
C    2,RFLX,EFLX,SFLX,VFLX,HFLX1,HFLW1,RA,RAC(NY,NX),RAG(NY,NX) 
C    3,RAGZ,RAR1,RAGX,RI,RAGS(NY,NX),BKVL(NUM(NY,NX),NY,NX)
C    3,VOLW1(NUM(NY,NX),NY,NX),VOLI1(NUM(NY,NX),NY,NX),RFLX1,ALBG 
C    4,RADXG(NY,NX),THRYG(NY,NX),THRMA,THRYW(NY,NX),THS(NY,NX)
C    5,PARG(NY,NX),VPQ(NY,NX),VP1,FRADG(NY,NX),THRMCX(NY,NX)
C    5,PSISM1(NUM(NY,NX),NY,NX),PSISO(NUM(NY,NX),NY,NX) 
C    6,FLQM,EVAP(NY,NX),PARE,PARS,PARSG(NY,NX),HWFLQM
...
C
      ...
      CNV1=THETPX(NUM(NY,NX),NY,NX)**2/POROQ(NUM(NY,NX),NY,NX)*XNPR
     2*WGSG1(NUM(NY,NX),NY,NX)
      IF(CVRD(NY,NX).GT.ZERO)THEN
      IF(CNVR.GT.ZERO.AND.CNV1.GT.ZERO)THEN
      AVCNVS=2.0*CNVR*CNV1
     2/(CNVR*DLYR(3,NUM(NY,NX),NY,NX)+CNV1*DLYRR(NY,NX))*CVRD(NY,NX)
      ELSE
      AVCNVS=2.0*CNVR
     2/(DLYR(3,NUM(NY,NX),NY,NX)+DLYRR(NY,NX))*CVRD(NY,NX)
      ENDIF
     ...
     5+0.611*THETIX(NUM(NY,NX),NY,NX)+WTHET1*THETPX(NUM(NY,NX),NY,NX))
      TCND1R=TCND1*XNPHR
      ATCNDR=2.0*TCNDR1*TCND1R/(TCNDR1*DLYR(3,NUM(NY,NX),NY,NX)
...
C
      ...
      FLVX=AVCNVS*(VPR-VP1)*AREA(3,NUM(NY,NX),NY,NX) 
      IF(FLVX.GE.0.0)THEN
      FLV2=AMIN1(FLVX,VOLWR2*XH(3,0,NY,NX))
      IF(HFLWX.GE.0.0)THEN
      FLV2=AMIN1(FLV2,HFLWX/(4.19*TKR1+VAP))
      ENDIF
      HWFLV2=(4.19*TKR1+VAP)*FLV2
      ELSE
      FLV2=AMAX1(FLVX,-VOLW12*XH(3,0,NY,NX))
      IF(HFLWX.LT.0.0)THEN
...
C
     ...
     2+1.9274*VOLI1(0,NY,NX)
      ...
      TKR1=(ENGYR+HFLR2+HWFLM2-HWFLV2-HFLCR2)/VHCPR2
      TKS1=TKS1+(HWFLV2+HFLCR2)/VHCP12
C     IF(I.EQ.328)THEN
C     WRITE(*,1111)'EFLXR2',I,J,M,NX,NY,NUM(NY,NX),N,TKR1,TKS1
C    2,TKQ(NY,NX),RFLXR2,EFLXR2,SFLXR2,VFLXR2,HFLR2,HWFLM2,HWFLV2 
C    3,HFLCR2,VHCPR2,HWFLV2,VHCP12,VOLWR2,VOLW12
C    3,FLVX,FLV2,FLVX,VPR,VP1,AVCNVS,CNVR,CNV1
C    3,PSISM1(0,NY,NX),PSISV1,THETWR,VOLWRX(NY,NX),TRC0(NY,NX)
C    4,VHCPRX(NY,NX),PARS,PARE,RA,RZR,RZ,RI,TKQ(NY,NX),VOLW1(0,NY,NX) 
C    5,VOLW1(NUM(NY,NX),NY,NX),VOLT(NUM(NY,NX),NY,NX),THRMZ2,HFLWX,FLV1 
...
C    6,TCND1,STC(NUM(NY,NX),NY,NX),THETWX(NUM(NY,NX),NY,NX),TCNDW1
...
C     FOR LATER UPDATES TO STATE VARIABLES
C
      FLWL(3,NUM(NY,NX),NY,NX)=FLQM+EVAP(NY,NX)+FLV1
      FLWLX(3,NUM(NY,NX),NY,NX)=FLQM+EVAP(NY,NX)+FLV1
      FLWHL(3,NUM(NY,NX),NY,NX)=FLHM
      HFLWL(3,NUM(NY,NX),NY,NX)=HWFLQM+HFLW1+HWFLV1+HFLCR1
      FLWRL(NY,NX)=FLYM+EVAPR(NY,NX)-FLV1
      HFLWRL(NY,NX)=HWFLYM+HFLR1-HWFLV1-HFLCR1
      FLWVL(NUM(NY,NX),NY,NX)=RFLWV(NY,NX)*(VOLW1(NUM(NY,NX),NY,NX)
     2-VOLWX1(NUM(NY,NX),NY,NX))
      FLWV(NUM(NY,NX),NY,NX)=FLWV(NUM(NY,NX),NY,NX)
     2+FLWVL(NUM(NY,NX),NY,NX)
C     IF(I.GE.110.AND.NX.EQ.2)THEN
C     WRITE(*,3376)'FLW1',I,J,M,NX,NY,NUM(NY,NX)
C    2,FLWL(3,NUM(NY,NX),NY,NX)
C    2,PSISM1(0,NY,NX),PSISM1(NUM(NY,NX),NY,NX),VOLWRX(NY,NX)
C    3,VOLW1(0,NY,NX),VOLW1(NUM(NY,NX),NY,NX)
C    3,THETWX(NUM(NY,NX),NY,NX)
C    2,FLQM,EVAP(NY,NX),PARE,VPQ(NY,NX),VP1,TK1(NUM(NY,NX),NY,NX)
C    4,FLWRL(NY,NX),FLYM,EVAPR(NY,NX),FLV1,VOLX(NUM(NY,NX),NY,NX)
C     WRITE(*,3376)'FLWHL',I,J,M,NX,NY,NUM(NY,NX)
C    2,FLWHL(3,NUM(NY,NX),NY,NX),FLHM
C    2,HFLWL(3,NUM(NY,NX),NY,NX),HFLW1,RFLX,EFLX,SFLX,VFLX 
C    2,HWFLQM,HWFLV1,HFLCR1,HFLWRL(NY,NX),HWFLYM
C    3,HFLR1,HWFLV1,HFLCR1,TK1(NUM(NY,NX),NY,NX)
3376  FORMAT(A8,6I4,40E12.4)
C     ENDIF
C
...
C
      ...
      CND1=HCND(3,1,NUM(NY,NX),NY,NX)*XNPH
      ELSE
      K1=MAX(1,MIN(100,INT(100.0*(AMAX1(0.0,POROS(NUM(NY,NX),NY,NX)
     2-THETWX(NUM(NY,NX),NY,NX)))/POROS(NUM(NY,NX),NY,NX))+1))
      CND1=HCND(3,K1,NUM(NY,NX),NY,NX)*XNPH
      ENDIF
      AVCND1=2.0*CNDR*CND1/(CNDR*DLYR(3,NUM(NY,NX),NY,NX)
     2+CND1*DLYRR(NY,NX)) 
      FLXQR=AVCND1*(PSISM1(0,NY,NX)-PSISM1(NUM(NY,NX),NY,NX))
     2*AREA(3,NUM(NY,NX),NY,NX)
     ...
     2*VOLP1(NUM(NY,NX),NY,NX))
      ...
      FLWRM(M,NY,NX)=FLXSR
C     IF(IYRC.EQ.2006.AND.I.EQ.361)THEN
C     WRITE(*,4322)'FLXSR',I,J,M,NX,NY,FLWRL(NY,NX)
...
C 
      ...
      IF(VOLW1(NUM(NY,NX),NY,NX)+VOLI1(NUM(NY,NX),NY,NX)
     2.GT.VOLA(NUM(NY,NX),NY,NX))THEN
      FLWLY=AMIN1(0.0,AMAX1(-XH(3,NUM(NY,NX),NY,NX)
     2*VOLW1(NUM(NY,NX),NY,NX),VOLA(NUM(NY,NX),NY,NX)
     3-VOLW1(NUM(NY,NX),NY,NX)-VOLI1(NUM(NY,NX),NY,NX)))
      HFLWLY=FLWLY*4.19*TK1(NUM(NY,NX),NY,NX)
      FLWL(3,NUM(NY,NX),NY,NX)=FLWL(3,NUM(NY,NX),NY,NX)+FLWLY 
      HFLWL(3,NUM(NY,NX),NY,NX)=HFLWL(3,NUM(NY,NX),NY,NX)+HFLWLY 
      FLWLYR=AMIN1(0.0,FLWLY+VOLPH1(NUM(NY,NX),NY,NX))
      HFLWYR=FLWLYR*4.19*TK1(NUM(NY,NX),NY,NX)
      FLWLYH=FLWLY-FLWLYR
      HFLWYH=FLWLYH*4.19*TK1(NUM(NY,NX),NY,NX)
      FLWRL(NY,NX)=FLWRL(NY,NX)-FLWLYR 
      HFLWRL(NY,NX)=HFLWRL(NY,NX)-HFLWYR
      FLWHL(3,NUM(NY,NX),NY,NX)=FLWHL(3,NUM(NY,NX),NY,NX)-FLWLYH 
      HFLWL(3,NUM(NY,NX),NY,NX)=HFLWL(3,NUM(NY,NX),NY,NX)-HFLWYH
C     IF(I.GE.110.AND.NX.EQ.2)THEN
C     WRITE(*,4323)'FLWLY',I,J,M,NX,NY,FLWRL(NY,NX)
C    2,FLWL(3,NUM(NY,NX),NY,NX),FLWLY
C    2,VOLAH1(NUM(NY,NX),NY,NX),VOLPH1(NUM(NY,NX),NY,NX)
C    2,VOLWH1(NUM(NY,NX),NY,NX),VOLIH1(NUM(NY,NX),NY,NX)
C    2,VOLAH1(NUM(NY,NX)+1,NY,NX),VOLPH1(NUM(NY,NX)+1,NY,NX)
C    2,VOLWH1(NUM(NY,NX)+1,NY,NX),VOLIH1(NUM(NY,NX)+1,NY,NX)
C    3,VOLW1(0,NY,NX)
4323  FORMAT(A8,5I4,30E12.4)
C     ENDIF
      ENDIF
      IF(VOLWH1(NUM(NY,NX),NY,NX)+VOLIH1(NUM(NY,NX),NY,NX)
     2.GT.VOLAH1(NUM(NY,NX),NY,NX))THEN
      FLWHY=AMIN1(0.0,AMAX1(-XH(3,NUM(NY,NX),NY,NX) 
     2*VOLWH1(NUM(NY,NX),NY,NX),VOLAH1(NUM(NY,NX),NY,NX)
     3-VOLWH1(NUM(NY,NX),NY,NX)-VOLIH1(NUM(NY,NX),NY,NX)))
      HFLWHY=FLWHY*4.19*TK1(NUM(NY,NX),NY,NX) 
      FLWHL(3,NUM(NY,NX),NY,NX)=FLWHL(3,NUM(NY,NX),NY,NX)+FLWHY 
      HFLWL(3,NUM(NY,NX),NY,NX)=HFLWL(3,NUM(NY,NX),NY,NX)+HFLWHY 
      FLWRL(NY,NX)=FLWRL(NY,NX)-FLWHY 
      HFLWRL(NY,NX)=HFLWRL(NY,NX)-HFLWHY 
C     IF(I.GE.110.AND.NX.EQ.2)THEN
C     WRITE(*,4324)'FLWHY',I,J,M,NX,NY,FLWRL(NY,NX)
C    2,FLWL(3,NUM(NY,NX),NY,NX),FLWHY
C    2,VOLAH1(NUM(NY,NX),NY,NX),VOLPH1(NUM(NY,NX),NY,NX)
C    2,VOLWH1(NUM(NY,NX),NY,NX),VOLIH1(NUM(NY,NX),NY,NX)
C    2,VOLAH1(NUM(NY,NX)+1,NY,NX),VOLPH1(NUM(NY,NX)+1,NY,NX)
C    2,VOLWH1(NUM(NY,NX)+1,NY,NX),VOLIH1(NUM(NY,NX)+1,NY,NX)
...
C
     ...
     3+VHCP1(0,NY,NX)*(TFREEZ-TK1(0,NY,NX))
     4-HFLWRL(NY,NX))
      IF(TFLX1.LT.0.0)THEN
      TFLX=AMAX1(-333.0*DENSI*VOLI1(0,NY,NX)*XH(3,0,NY,NX),TFLX1)
      ELSE
      TFLX=AMIN1(333.0*VOLW1(0,NY,NX)*XH(3,0,NY,NX),TFLX1)
      ENDIF
      WFLX=-TFLX/333.0
...
C
      ...
      IF((TK1(NUM(NY,NX),NY,NX).LT.TFREEZ
     2.AND.VOLW1(NUM(NY,NX),NY,NX).GT.ZERO*VOLA(NUM(NY,NX),NY,NX)
     3.AND.VOLI1(NUM(NY,NX),NY,NX).LT.VOLA(NUM(NY,NX),NY,NX))
     4.OR.(TK1(NUM(NY,NX),NY,NX).GT.TFREEZ
     5.AND.VOLI1(NUM(NY,NX),NY,NX).GT.ZERO*VOLA(NUM(NY,NX),NY,NX)))THEN
      TFLX1=FGRD(NUM(NY,NX),NY,NX)*(1.0/(1.0+TFREEZ*6.2913E-03) 
     2*(TFREEZ*4.19*(FLWL(3,NUM(NY,NX),NY,NX)+FLWHL(3,NUM(NY,NX),NY,NX))
     3+VHCP1(NUM(NY,NX),NY,NX)*(TFREEZ-TK1(NUM(NY,NX),NY,NX))
     4-HFLWL(3,NUM(NY,NX),NY,NX)))
      IF(TFLX1.LT.0.0)THEN
      TFLX=AMAX1(-333.0*DENSI*VOLI1(NUM(NY,NX),NY,NX)
     2*XH(3,NUM(NY,NX),NY,NX),TFLX1)
      ELSE
      TFLX=AMIN1(333.0*VOLW1(NUM(NY,NX),NY,NX)
     2*XH(3,NUM(NY,NX),NY,NX),TFLX1)
      ENDIF
      WFLX=-TFLX/333.0
      IF(WFLX.GT.0.0.AND.VOLI1(NUM(NY,NX),NY,NX)
     2.GT.ZEROS(NY,NX))THEN
      WFLXL(3,NUM(NY,NX),NY,NX)=WFLX 
      ELSEIF(WFLX.LT.0.0.AND.VOLW1(NUM(NY,NX),NY,NX)
     2.GT.ZEROS(NY,NX))THEN
      WFLXL(3,NUM(NY,NX),NY,NX)=WFLX 
      ELSE
      TFLX=0.0
      WFLXL(3,NUM(NY,NX),NY,NX)=0.0
      ENDIF
      ELSE
      TFLX=0.0
      WFLXL(3,NUM(NY,NX),NY,NX)=0.0
...
C     SURFACE HEAT STORAGE
C
      IF((TK1(NUM(NY,NX),NY,NX).LT.273.15.AND.VOLWH1(NUM(NY,NX),NY,NX)
     2.GT.ZERO*VOLT(NUM(NY,NX),NY,NX)).OR.(TK1(NUM(NY,NX),NY,NX)
     3.GT.273.15.AND.VOLIH1(NUM(NY,NX),NY,NX)
     4.GT.ZERO*VOLT(NUM(NY,NX),NY,NX)))THEN
      TFLX1=FMAC(NUM(NY,NX),NY,NX)*(1.0/(1.0+273.15*6.2913E-03) 
     2*(273.15*4.19*(FLWL(3,NUM(NY,NX),NY,NX)+FLWHL(3,NUM(NY,NX),NY,NX))
     3+VHCP1(NUM(NY,NX),NY,NX)*(273.15-TK1(NUM(NY,NX),NY,NX))
     4-HFLWL(3,NUM(NY,NX),NY,NX)))
      IF(TFLX1.LT.0.0)THEN
      TFLXH=AMAX1(-333.0*DENSI*VOLIH1(NUM(NY,NX),NY,NX)
     2*XH(3,NUM(NY,NX),NY,NX),TFLX1)
      ELSE
      TFLXH=AMIN1(333.0*VOLWH1(NUM(NY,NX),NY,NX)
     2*XH(3,NUM(NY,NX),NY,NX),TFLX1)
      ENDIF
      WFLXH=-TFLXH/333.0
      IF(WFLXH.GT.0.0.AND.VOLIH1(NUM(NY,NX),NY,NX)
     2.GT.ZEROS(NY,NX))THEN
      WFLXLH(3,NUM(NY,NX),NY,NX)=WFLXH 
      ELSEIF(WFLXH.LT.0.0.AND.VOLWH1(NUM(NY,NX),NY,NX)
     2.GT.ZEROS(NY,NX))THEN
      WFLXLH(3,NUM(NY,NX),NY,NX)=WFLXH 
      ELSE
      TFLXH=0.0
      WFLXLH(3,NUM(NY,NX),NY,NX)=0.0
      ENDIF
      ELSE
      TFLXH=0.0
      WFLXLH(3,NUM(NY,NX),NY,NX)=0.0
      ENDIF
      TFLXL(3,NUM(NY,NX),NY,NX)=TFLX+TFLXH
C     IF(NY.EQ.1)THEN
C     WRITE(*,4358)'TFLX',I,J,M,TFREEZ,TK1(NUM(NY,NX),NY,NX),PSISV1 
C    2,TFLX,TFLXH,TFLXL(3,NUM(NY,NX),NY,NX),WFLX,WFLXH
C    2,WFLXL(3,NUM(NY,NX),NY,NX),WFLXLH(3,NUM(NY,NX),NY,NX)
C    4,VOLW1(NUM(NY,NX),NY,NX),VOLWH1(NUM(NY,NX),NY,NX)
C    4,VOLI1(NUM(NY,NX),NY,NX),VOLIH1(NUM(NY,NX),NY,NX)
C    5,FGRD(NUM(NY,NX),NY,NX),FMAC(NUM(NY,NX),NY,NX)
...
C
     ...
     2,EXP(-13.650-0.857*LOG(-PSISM1(0,NY,NX))))
      ...
      ENDIF
C     IF(BKVL(NUM(NY,NX),NY,NX).GT.0.0)THEN
...
C
      ...
      THAW(3,NUM(NY,NX),NY,NX)=THAW(3,NUM(NY,NX),NY,NX)
     2+WFLXL(3,NUM(NY,NX),NY,NX)
      THAWH(3,NUM(NY,NX),NY,NX)=THAWH(3,NUM(NY,NX),NY,NX)
     2+WFLXLH(3,NUM(NY,NX),NY,NX)
      HTHAW(3,NUM(NY,NX),NY,NX)=HTHAW(3,NUM(NY,NX),NY,NX)
     2+TFLXL(3,NUM(NY,NX),NY,NX)
...
C
     ...
     2.AND.HYST(NUM(NY,NX),NY,NX).GT.ZERO)THEN
      HYST(NUM(NY,NX),NY,NX)=AMIN1(1.0,AMAX1(0.0,HYST(NUM(NY,NX),NY,NX)
...
C
      IF(VOLWH1(NUM(NY,NX),NY,NX).GT.ZEROS(NY,NX))THEN
      FINHX=XNPH*6.283*HCND(2,1,NUM(NY,NX),NY,NX)
     2*AREA(N,NUM(NY,NX),NY,NX)
     2*(PSISE(NUM(NY,NX),NY,NX)-PSISM1(NUM(NY,NX),NY,NX))
     3/LOG(PHOL(NUM(NY,NX),NY,NX)/HRAD(NUM(NY,NX),NY,NX)) 
      IF(FINHX.GT.0.0)THEN
      FINHL(NUM(NY,NX),NY,NX)=AMAX1(0.0,AMIN1(FINHX
     2,XH(3,NUM(NY,NX),NY,NX)*VOLWH1(NUM(NY,NX),NY,NX)
     3,VOLPX1(NUM(NY,NX),NY,NX)))
      ELSE
      FINHL(NUM(NY,NX),NY,NX)=AMIN1(0.0,AMAX1(FINHX
     2,-VOLPH1(NUM(NY,NX),NY,NX)
     3,-XH(3,NUM(NY,NX),NY,NX)*VOLW1(NUM(NY,NX),NY,NX)))
      ENDIF
      FINHM(M,NUM(NY,NX),NY,NX)=FINHL(NUM(NY,NX),NY,NX)
      FINH(NUM(NY,NX),NY,NX)=FINH(NUM(NY,NX),NY,NX)
     2+FINHL(NUM(NY,NX),NY,NX)
C     IF(J.EQ.12.AND.M.EQ.1)THEN
C     WRITE(*,3367)'HOLE',I,J,M,NX,NY
C    2,FINHL(NUM(NY,NX),NY,NX),FINHX
C    2,VOLWH1(NUM(NY,NX),NY,NX),VOLPH1(NUM(NY,NX),NY,NX)
C    3,VOLAH1(NUM(NY,NX),NY,NX),PSISE(NUM(NY,NX),NY,NX)
C    4,PSISM1(NUM(NY,NX),NY,NX),VOLW1(NUM(NY,NX),NY,NX)
C    5,HCND(2,1,NUM(NY,NX),NY,NX),PHOL(NUM(NY,NX),NY,NX)
C    5,HRAD(NUM(NY,NX),NY,NX) 
3367  FORMAT(A8,5I4,20E12.4)
C     ENDIF
      ELSE
      FINHM(M,NUM(NY,NX),NY,NX)=0.0
      FINHL(NUM(NY,NX),NY,NX)=0.0
      ENDIF
C
...
C
      ...
      IF(N3.GE.NUM(N2,N1).AND.N6.GE.NU(N5,N4)
...
C
      ...
      FINHX=XNPH*6.283*HCND(2,1,N6,N5,N4)*AREA(N,N6,N5,N4)
     2*(PSISE(N6,N5,N4)-PSISM1(N6,N5,N4))
     3/LOG(PHOL(N6,N5,N4)/HRAD(N6,N5,N4)) 
      IF(FINHX.GT.0.0)THEN
      FINHL(N6,N5,N4)=AMAX1(0.0,AMIN1(FINHX,XH(N,N6,N5,N4) 
     2*VOLWH1(N6,N5,N4),VOLPX1(N6,N5,N4)))
      ELSE
      FINHL(N6,N5,N4)=AMIN1(0.0,AMAX1(FINHX,-VOLPH1(N6,N5,N4)
     2,-XH(N,N6,N5,N4)*VOLW1(N6,N5,N4)))
      ENDIF
...
C
     ...
     2/(VHCP1(N3,N2,N1)+VHCP1(N6,N5,N4))
      ...
      IF(FLVX.GE.0.0)THEN
      FLVL=AMIN1(FLVX,VOLW1(N3,N2,N1)*XH(N,N3,N2,N1))
      IF(HFLWX.GE.0.0)THEN
      FLVL=AMIN1(FLVL,HFLWX/(4.19*TK1(N3,N2,N1)+VAP))
      ENDIF
      HWFLVL=(4.19*TK1(N3,N2,N1)+VAP)*FLVL
      ELSE
      FLVL=AMAX1(FLVX,-VOLW1(N6,N5,N4)*XH(N,N6,N5,N4))
      IF(HFLWX.LT.0.0)THEN
...
C
      IF(VHCP1(N3,N2,N1).GT.ZEROS(NY,NX))THEN
      IF(N3.EQ.NUM(NY,NX))THEN
      TK1X=TK1(N3,N2,N1)-(HWFLVL-HFLX1)/VHCP1(N3,N2,N1)
      ELSE
     ...
     2/(VHCP1(N3,N2,N1)+VHCP1(N6,N5,N4))
      ...
      IF(HFLWC.GE.0.0)THEN
      HFLWS=AMAX1(0.0,AMIN1(HFLWX,HFLWC))
      ELSE
      HFLWS=AMIN1(0.0,AMAX1(HFLWX,HFLWC))
      ENDIF
      HFLWL(N,N6,N5,N4)=HWFLWL+HWFLHL+HFLWS
C     IF(I.EQ.346)THEN
C     WRITE(*,8765)'HFLWL',I,J,M,N1,N2,N3,N4,N5,N6,N,HFLWL(N,N3,N2,N1)
C    2,HFLWL(N,N6,N5,N4),HWFLWL,HWFLHL,HFLWC,HFLWX,HFLWS,TCND1,TCND2
C    3,ATCND1,TK1X,TKLX,TKY,HWFLVL,TK1(N3,N2,N1),TK1(N6,N5,N4)
...
C    4,STC(N6,N5,N4),THETWX(N6,N5,N4),TCNDW2
C    2,THETIX(N6,N5,N4) 
...
C     IF(VOLW1(N6,N5,N4)+VOLI1(N6,N5,N4).GT.VOLA(N6,N5,N4))THEN
C     FLWLY=AMIN1(0.0,AMAX1(-XH(N,N6,N5,N4)*VOLW1(N6,N5,N4) 
C    2,VOLA(N6,N5,N4)-VOLW1(N6,N5,N4)-VOLI1(N6,N5,N4)))
C     IF(BKDS(N3,N2,N1).GT.ZERO)FLWLY=AMAX1(FLWLY,-VOLP1(N3,N2,N1))
C     HFLWLY=FLWLY*4.19*TK1(N6,N5,N4)
C     FLWL(N,N6,N5,N4)=FLWL(N,N6,N5,N4)+FLWLY 
C     HFLWL(N,N6,N5,N4)=HFLWL(N,N6,N5,N4)+HFLWLY 
C     IF(I.EQ.336)THEN
C     WRITE(*,1115)'FLWLY',I,J,M,N1,N2,N3,N4,N5,N6,N,FLWL(N,N3,N2,N1)
C    2,FLWL(N,N6,N5,N4),FLW(N,N3,N2,N1),FLW(N,N6,N5,N4)
C    3,FLWLY,XH(N,N6,N5,N4),VOLW1(N6,N5,N4) 
C    2,VOLA(N6,N5,N4),VOLW1(N6,N5,N4),VOLI1(N6,N5,N4),VOLP1(N3,N2,N1)
C     ENDIF
C     ENDIF
C     IF(VOLWH1(N6,N5,N4)+VOLIH1(N6,N5,N4).GT.VOLAH1(N6,N5,N4))THEN
C     FLWHY=AMIN1(0.0,AMAX1(-XH(N,N6,N5,N4)*VOLWH1(N6,N5,N4) 
C    2,-VOLPH1(N3,N2,N1),VOLAH1(N6,N5,N4)-VOLWH1(N6,N5,N4)
C    3-VOLIH1(N6,N5,N4)))
C     HFLWHY=FLWHY*4.19*TK1(N6,N5,N4) 
C     FLWHL(N,N6,N5,N4)=FLWHL(N,N6,N5,N4)+FLWHY 
C     HFLWL(N,N6,N5,N4)=HFLWL(N,N6,N5,N4)+HFLWHY 
C     ENDIF
...
C
     ...
     3-HFLWL(N,N6,N5,N4)))
      IF(TFLX1.LT.0.0)THEN
      TFLX=AMAX1(-333.0*DENSI*VOLI1(N6,N5,N4)*XH(N,N6,N5,N4),TFLX1)
      ELSE
      TFLX=AMIN1(333.0*VOLW1(N6,N5,N4)*XH(N,N6,N5,N4),TFLX1)
      ENDIF
      WFLX=-TFLX/333.0
...
C
     ...
     3-HFLWL(N,N6,N5,N4)))
      IF(TFLX1.LT.0.0)THEN
      TFLXH=AMAX1(-333.0*DENSI*VOLIH1(N6,N5,N4)*XH(N,N6,N5,N4),TFLX1)
      ELSE
      TFLXH=AMIN1(333.0*VOLWH1(N6,N5,N4)*XH(N,N6,N5,N4),TFLX1)
      ENDIF
      WFLXH=-TFLXH/333.0
...
C     ENDIF
      ...
      TFLXL(N,N3,N2,N1)=0.0
      WFLXL(N,N3,N2,N1)=0.0
      WFLXLH(N,N3,N2,N1)=0.0 
...
C
     ...
     2.AND.CDPTH(0,N2,N1).LE.CDPTHI(N2,N1))THEN
...
C     RUNON
C
      QRX1=AMIN1(0.0,DTBLX(N2,N1)+TVOLZ1/AREA(3,NUM(N2,N1),N2,N1))
...
C
     ...
     2*(1.0-AREAU(N3,N2,N1))/(RCHGFU+1.0)*RCHGFT 
      FLWTHL=AMAX1(FLWTH,AMIN1(0.0,-XH(N,N3,N2,N1)*(VOLWH1(N3,N2,N1) 
...
C    2.AND.DPTHA(N2,N1).GT.DTBLX(N2,N1)
     2.AND.(BKDS(N3,N2,N1).LE.ZERO.OR.VOLP2.GT.0.0))THEN
      PSISWD=XN*0.005*SLOPE(N,N2,N1)*DLYR(N,N3,N2,N1)
     2*(1.0-DTBLG(N2,N1))
...
C    2.AND.DPTHA(N2,N1).GT.DTBLX(N2,N1)
     ...
     2*AREAU(N3,N2,N1)/(RCHGFU+1.0)*RCHGFT 
      FLWUHL=AMIN1(FLWUH,AMAX1(0.0,XH(N,N3,N2,N1)*(VOLPH2
     2-FLWHL(3,N3,N2,N1)+FLWHL(3,N3+1,N2,N1)+FINHL(N3,N2,N1))))
...
C    2,TFLXR(NY,NX),THQR1(NY,NX),VHCP1(0,NY,NX),VOLW1(0,NY,NX) 
...
C
      VOLI1(NUM(NY,NX),NY,NX)=VOLI1(NUM(NY,NX),NY,NX)+FLSI1(NY,NX)
     ...
     3+1.9274*(VOLI1(NUM(NY,NX),NY,NX)+VOLIH1(NUM(NY,NX),NY,NX))
      ...
      TK1(NUM(NY,NX),NY,NX)=(ENGY1+HFLSI1(NY,NX))
     2/VHCP1(NUM(NY,NX),NY,NX)
      ELSE
      TK1(NUM(NY,NX),NY,NX)=TKQ(NY,NX)
      ENDIF
C     IF(I.EQ.168)THEN
C     WRITE(*,7755)'TQR',I,J,M,NX,NY,VOLW1(NUM(NY,NX),NY,NX)
C    2,VOLWH1(NUM(NY,NX),NY,NX),TQR1(NY,NX) 
C     WRITE(*,7755)'TK1',I,J,M,NX,NY,TK1(NUM(NY,NX),NY,NX)
C    2,VHCP1(NUM(NY,NX),NY,NX),VHCM(NUM(NY,NX),NY,NX)
C    2,ENGY1,THQR1(NY,NX),HFLSI1(NY,NX),TQR1(NY,NX) 
C    3,VOLW1(NUM(NY,NX),NY,NX),VOLWH1(NUM(NY,NX),NY,NX)
C    4,VOLI1(NUM(NY,NX),NY,NX),FLSI1(NY,NX)
...
C    3,VOLP1(L,NY,NX),VOLPH1(L,NY,NX),VOLT(L,NY,NX),VOLX(L,NY,NX)
C    4,VOLA(L,NY,NX),VOLAH(L,NY,NX),DLYR(3,L,NY,NX)
...
C     WRITE(*,3377)'TKL',I,J,M,NX,NY,L,N6X(NY,NX),TK1(L,NY,NX),ENGY1
...
C    5,HFLSI1(NY,NX),HFLWL(3,L,NY,NX),HFLWL(3,N6X(NY,NX),NY,NX)
