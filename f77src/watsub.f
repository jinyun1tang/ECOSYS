...
C
     ...
     2,FVOLAH=0.0,PSISX=-0.5,PSISXR=-0.5
...
C     IF(BKVL(NU(NY,NX),NY,NX).GT.0.0)THEN
      ...
      PSISM1(NU(NY,NX),NY,NX)=AMAX1(PSIHY,-EXP(PSIMX(NY,NX)
...
C     DDLYR=AMIN1(DLYR(3,L,NY,NX),(VOLA(L,NY,NX)-(VOLW1(L,NY,NX)
C    2+VOLI1(L,NY,NX)))/AREA(3,L,NY,NX))
...
C     YDPTH(LL,NY,NX)=ALT(NY,NX)-DPTH(LL,NY,NX) 
905   CONTINUE
C     DLYR(3,L,NY,NX)=(CDPTH(L,NY,NX)-CDPTH(L-1,NY,NX))
...
C     VOLP1(L,NY,NX)=AMAX1(0.0,VOLA(L,NY,NX)-VOLW1(L,NY,NX)
C    2-VOLI1(L,NY,NX))
...
C     IF(BKVL(N3,N2,N1).GT.ZERO)THEN
...
C     ENDIF
C     IF(BKVL(N6,N5,N4).GT.ZERO)THEN
...
C     WRITE(*,1115)'FLWL',I,J,M,N4,N5,N6,N,FLWL(N,N3,N2,N1)
...
C    9,VHCP1(N3,N2,N1),VHCP1(N6,N5,N4),POROS(N6,N5,N4),THETAL 
1115  FORMAT(A8,7I4,60E12.4)
C     ENDIF
C
