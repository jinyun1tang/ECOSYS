...
C     DIMENSION VCO2(400,366,05)
     ...
     2,FRTX=1.0/(1.0-(1.0-FSTK)**2))
      ...
      PARAMETER(EN2F=0.20,VMXO=1.00,SPNDL=1.0E-04,CCNKM=1.0E-02
     2,CCNKX=1.0E+02,WTNDI=0.001,RCCZR=0.0557,RCCYR=0.167
     3,RCCZN=0.167,RCCYN=0.833)
      DATA RCCZ/0.167,0.167,0.0557,0.167/
      DATA RCCY/0.333,0.333,0.333,0.333/
      DATA RCCX/0.417,0.833,0.833/
      DATA RCCQ/0.833,0.833,0.833/
      DATA RTSK/0.50,1.0,4.0/
      DATA FXRN/1.00,0.10,0.50,0.05/
...
C    2,ICHK1(N,NR),RTDP1(N,NR,NZ,NY,NX),RTDP1X
...
C     ENDIF
      ...
      IF(RTDP1X.LE.CDPTHZ(L,NY,NX))THEN
      ICHK1(N,NR)=1
C
...
C    8,RDNDLC,RDNDLN,RDNDLP,WFR(1,L,NZ,NY,NX)  
...
C    3,TCSNC(NZ,NY,NX),TCUPTK(NZ,NY,NX),RSETC(NZ,NY,NX),WTSTG(NZ,NY,NX) 
C    2,THVSTC(NZ,NY,NX),HVSTC(NZ,NY,NX),VCO2F(NZ,NY,NX),VCH4F(NZ,NY,NX)
C    5,CARBN(NZ,NY,NX),TCO2T(NZ,NY,NX)  
